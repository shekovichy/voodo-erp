...
## 🤖 بوت واتساب (functions/) — نظام منفصل عن src/

مجلد `functions/` مشروع Cloud Functions قائم بذاته — سيرفر Node، مش جزء
من `build.py` ولا `index.html`. بيقرا نفس مشروع Firebase (`nexus-2fec6`)
بصلاحيات Admin SDK (بتتخطى `firestore.rules` بالكامل — الأمان هنا
مسؤولية كود الفنكشن نفسه). التفاصيل والنشر في `functions/README.md`.

**مرحلة 1 بس دلوقتي**: رد بسعر وتوفر المنتج، بدون AI — بحث مباشر في
`pos_data/inventory/branches/*`. مرحلة 2 (حالة الفاتورة) ومرحلة 3 (شات
حر بـ Claude API) لسه ماتعملتش.

⚠️ `functions/` ليها `npm install` و`package.json` خاص بيها منفصل عن أي
حاجة في `src/` — مالهاش علاقة بـ `python build.py` خالص.
...
- **"ضيف `email_verified == true` لـ `isOwner()`"** — سؤال سليم، لكن وقتها حساب المالك كان `emailVerified: false` فعلياً. تنفيذها ساعتها كان هيقفل المالك بره كل حاجة لحظة النشر. ✅ **اتنفذت في 2026-08-15** بعد ما الإيميل اتفعّل واتأكد بـ `auth_get_users` — الترتيب اللي مشينا بيه: تفعيل الإيميل → logout/login (التوكن بيفضل شايل القيمة القديمة لحد ساعة) → تأكيد `email_verified` من `getIdTokenResult()` → نشر. الحساب مالوش `roles/{uid}` doc، فـ `isOwner()` هو مصدر صلاحيته الوحيد.
