      ...
      _saMonths.forEach(month => {
        _db.collection('pos_sales').doc(month).collection('branches').doc(b)
          .onSnapshot(snap => {
            ...
            // Fall back to the local cache on a denied/failed read — same
            // fix as the inventory listener above (see its comment for the
            // full story: a listener with no fallback here left this
            // branch+month's slice of _salesCache simply unset instead of
            // showing the last known-good local data).
            console.error(`Firestore sales/${month}/${b} error:`, err);
            const localSalesAll = DB.g('sales', []);
            const localMonthBranch = localSalesAll.filter(s => s.date.slice(0, 7) === month && (s.branchId || currentBranch) === b);
            _salesCache = [
              ..._salesCache.filter(s => !(s.date.slice(0, 7) === month && (s.branchId || '') === b)),
