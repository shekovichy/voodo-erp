...
// Pending requests expire the same way. A request nobody answered piles up in
// the admin's queue forever otherwise, and approving a week-old one would put
// that discount live today — the stale-approval problem in a different shape.
//
// Requests written by the previous build carry no expiresAt (it was only set
// at approval time back then), so fall back to the end of the day they were
// raised. Treating them as immortal instead would leave exactly the stale
// queue this is meant to drain.
function _pendingExpiryOf(r) {
  return r.expiresAt || (r.date ? _endOfDayMs(r.date) : 0);
}
function isPendingLive(r) {
  return r.status === 'pending' && Date.now() < _pendingExpiryOf(r);
}
// Flip anything already past its day to 'expired' so it stops coming back in
// the listener query at all, instead of being filtered out on every render
// forever. Safe to run from any device: the rule only permits pending ->
// expired after expiry, and only for the requester or an admin.
function _expireStaleRequests() {
  getApprovals()
    .filter(function(r){ return r.status === 'pending' && Date.now() >= _pendingExpiryOf(r); })
    .forEach(function(r){
      r.status    = 'expired';
      r.expiredAt = new Date().toISOString();
      if (!r.expiresAt) r.expiresAt = _pendingExpiryOf(r);   // backfill legacy
      _saveApproval(r);
    });
}
...
function sendForApproval() {
  ...
  showPromptModal('ملاحظة للمدير (اختياري):', '', function(val) {
    ...
    var request = {
      ...
      note: note,
      status: 'pending',
      // A request nobody answers dies with the day too, not just an approved
      // one. Reset to the approval day in _decideRequest.
      expiresAt: _endOfDayMs(),
      adminNote: ''
    };
...
function renderApprovalsList() {
  var list = getApprovals().filter(isPendingLive);
...
function _decideRequest(id, decision, noteElId, repaint) {
  ...
  if (!req || !isPendingLive(req)) return;
  req.status    = decision;
...
function updateApprovalBadge() {
  var pending = getApprovals().filter(isPendingLive).length;
