...
// ── Month keys must use the SHOP's calendar, not UTC ────────────────
// A sale stores `date: new Date().toISOString()` — correct to store, wrong to
// slice. Cairo runs UTC+2/+3, so 01:30 on the 1st of a month is 22:30 on the
// previous day in UTC, and `date.slice(0,7)` filed it under the previous
// month. Anything that buckets or filters by month has to go through here.
//
// Only for ISO timestamps. Fields typed into a date input (expenses,
// attendance) are already 'YYYY-MM-DD' in local terms — slicing THOSE is
