...
// ââ CLOUD DATA CACHES â synced with Firestore in real-time ââââââââââââââ
const BRANCH_IDS      = ['wh','b1','b2','b3','b4'];
const BRANCH_DEFAULTS = { wh:'ð­ Ø§ÙÙØ®Ø²Ù Ø§ÙØ±Ø¦ÙØ³Ù', b1:'Ø§ÙÙØ±Ø¹ Ø§ÙØ£ÙÙ', b2:'Ø§ÙÙØ±Ø¹ Ø§ÙØ«Ø§ÙÙ', b3:'Ø§ÙÙØ±Ø¹ Ø§ÙØ«Ø§ÙØ«', b4:'Ø§ÙÙØ±Ø¹ Ø§ÙØ±Ø§Ø¨Ø¹' };
let currentBranch       = DB.g('currentBranch', 'b1');
let _invCacheByBranch   = {};                        // { b1:[], b2:[], b3:[], b4:[] }
let _salesCache         = [];                        // filled by Firebase listeners â each sale has .branchId
let _settingsCache      = { threshold: DB.g('threshold', 5), salespeople: DB.g('salespeople', ['ÙØ­ÙØ¯','Ø§ÙØ§Ø¡']) };
...
// SALES â addSale() ÙØ¥Ø¶Ø§ÙØ© ÙØ§ØªÙØ±Ø© / setSales([]) ÙÙØ³Ø­ Ø§ÙÙÙ
...
// USERS â kept in localStorage (passwords stay local)
const getUsers = ()  => DB.g('users', { admin: '', cashier: '' });
function setUsers(v) {
  setUsersLocal(v); // passwords NEVER go to Firestore
}

// Branch-specific cashier users (ÙÙØ²Ø± + Ø¨Ø§Ø³ÙØ±Ø¯ ÙØ®ØªÙÙ ÙÙÙ ÙØ±Ø¹)
...
const getSalespeople  = () => _settingsCache.salespeople && _settingsCache.salespeople.length ? _settingsCache.salespeople : ['ÙØ­ÙØ¯','Ø§ÙØ§Ø¡'];
...
async function doLogin() {
  ...
  if (!pass) { document.getElementById('loginError').textContent = 'Ø£Ø¯Ø®Ù ÙÙÙØ© Ø§ÙÙØ±ÙØ±'; document.getElementById('loginError').classList.remove('hidden'); return; }

  // First-run: no admin password set yet â show setup
  if (!users.admin) {
    showFirstRunSetup();
    return;
  }
  ...
  if (user === 'admin' && await checkPass(pass, users.admin)) {
    ...
    initFirebase();   // â ÙØ´ØºÙÙ Firebase Ø¹ÙØ¯ Ø¯Ø®ÙÙ Ø§ÙØ£Ø¯ÙÙ
    initBranchUI();
    document.getElementById('topbarLogout').style.display = 'inline-flex';
    showPage('home');
    setTimeout(() => addAuditLog('auth.login', 'ØªØ³Ø¬ÙÙ Ø¯Ø®ÙÙ: admin', currentBranch), 500);
    ...
    if (matchedBranch) {
      ...
      if (matchedBranch === 'wh') {
        ...
        initFirebase();
        showPage('warehouse');
        setTimeout(() => addAuditLog('auth.login', `ØªØ³Ø¬ÙÙ Ø¯Ø®ÙÙ ÙØ®Ø²Ù: ${user}`, matchedBranch), 500);
      } else {
        document.getElementById('managerView').classList.remove('hidden');
        initFirebase();
        initBranchUI();
        showPage('home');
        renderHomeIcons();
        setTimeout(() => addAuditLog('auth.login', `ØªØ³Ø¬ÙÙ Ø¯Ø®ÙÙ ÙØ§Ø´ÙØ±: ${user} â ${getBranchName(matchedBranch)}`, matchedBranch), 500);
      }
    } else {
...
// ââ EXPORT REPORTS â Excel & PDF ââââââââââââââââââââââââââââ
function exportReportExcel(type) {
  ...
  if (type === 'sales') {
    ...
    const rows = [['Ø§ÙÙÙØªØ¬','Ø§ÙÙÙÙØ© Ø§ÙÙØ¨Ø§Ø¹Ø©','Ø§ÙØ¥ÙØ±Ø§Ø¯ (Ø¬.Ù)']];
    ...
    // Add summary at top
    const summary = [
      ['Ø¥Ø¬ÙØ§ÙÙ Ø§ÙÙØ¨ÙØ¹Ø§Øª', document.getElementById('rs-total')?.textContent || ''],
      ['ØµØ§ÙÙ Ø§ÙØ¥ÙØ±Ø§Ø¯Ø§Øª',  document.getElementById('rs-net')?.textContent || ''],
      ['Ø¹Ø¯Ø¯ Ø§ÙÙÙØ§ØªÙØ±',    document.getElementById('rs-count')?.textContent || ''],
      ['ÙØªÙØ³Ø· Ø§ÙÙØ§ØªÙØ±Ø©',  document.getElementById('rs-avg')?.textContent || ''],
      [],
      ...rows
    ];
    ws = XLSX.utils.aoa_to_sheet(summary);
    sheetName = 'ØªÙØ±ÙØ± Ø§ÙÙØ¨ÙØ¹Ø§Øª';
  } else if (type === 'profit') {
    const rows = [['Ø§ÙÙÙØªØ¬','Ø§ÙÙÙÙØ©','Ø§ÙØ¥ÙØ±Ø§Ø¯','Ø§ÙØªÙÙÙØ©','Ø§ÙØ±Ø¨Ø­','ÙØ§ÙØ´%']];
    document.querySelectorAll('#rp-breakdown tr').forEach(tr => {
      const cells = [...tr.querySelectorAll('td')].map(td => td.textContent.trim());
      if (cells.length) rows.push(cells);
    });
    const summary = [
      ['Ø¥Ø¬ÙØ§ÙÙ Ø§ÙØ¥ÙØ±Ø§Ø¯Ø§Øª', document.getElementById('rp-revenue')?.textContent || ''],
      ['Ø¥Ø¬ÙØ§ÙÙ Ø§ÙØªÙÙÙØ©',   document.getElementById('rp-cost')?.textContent || ''],
      ['ØµØ§ÙÙ Ø§ÙØ±Ø¨Ø­',       document.getElementById('rp-profit')?.textContent || ''],
      ['ÙØ§ÙØ´ Ø§ÙØ±Ø¨Ø­',       document.getElementById('rp-margin')?.textContent || ''],
      ['Ø¥Ø¬ÙØ§ÙÙ Ø§ÙØ®ØµÙÙØ§Øª', document.getElementById('rp-discounts')?.textContent || ''],
      [],
      ...rows
    ];
    ws = XLSX.utils.aoa_to_sheet(summary);
    sheetName = 'ØªÙØ±ÙØ± Ø§ÙØ£Ø±Ø¨Ø§Ø­';
  }

  if (!ws) { showToast('â ï¸ ÙØ§ ØªÙØ¬Ø¯ Ø¨ÙØ§ÙØ§Øª ÙÙØªØµØ¯ÙØ±'); return; }
  ...
  showToast('â ØªÙ ØªØµØ¯ÙØ± ' + sheetName + ' ÙÙ Excel');
}
...
function exportReportPDF(sectionId, title) {
  ...
  printWin.document.write(`<!DOCTYPE html><html dir="rtl"><head>
    ...
    <h1>${title}</h1>
    <div class="print-date">ð ØªØ§Ø±ÙØ® Ø§ÙØ·Ø¨Ø§Ø¹Ø©: ${new Date().toLocaleDateString('ar-EG', {weekday:'long',year:'numeric',month:'long',day:'numeric'})}</div>
    ${section.innerHTML}
  </body></html>`);
...
// ââ GOOGLE DRIVE BACKUP âââââââââââââââââââââââââââââââââââââ
...
function saveGdriveClientId() {
  ...
  if (!val) { showToast('â ï¸ Ø£Ø¯Ø®Ù Ø§ÙÙ Client ID Ø£ÙÙØ§Ù'); return; }
  DB.s('gdriveClientId', val);
  showToast('â ØªÙ Ø­ÙØ¸ Ø§ÙÙ Client ID');
  initGoogleDriveUI();
}
...
function initGoogleDriveUI() {
  ...
  if (statusEl) statusEl.textContent = _gdriveToken ? 'â ÙØªØµÙ Ø¨Ù Google Drive' : '';
}

function connectGoogleDrive() {
  const clientId = getGdriveClientId();
  if (!clientId) { showToast('â ï¸ Ø£Ø¯Ø®Ù Ø§ÙÙ Client ID Ø£ÙÙØ§Ù'); return; }
  ...
  const timer = setInterval(() => {
    try {
      ...
      if (hash && hash.includes('access_token')) {
        ...
        initGoogleDriveUI();
        showToast('â ØªÙ Ø§ÙØ±Ø¨Ø· ÙØ¹ Google Drive!');
...
async function backupToGoogleDrive(silent) {
  if (!_gdriveToken) { if (!silent) showToast('â ï¸ Ø§Ø±ØªØ¨Ø· Ø¨Ù Google Drive Ø£ÙÙØ§Ù'); return false; }
  const backup = {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (err.error && err.error.code===401) { _gdriveToken=null; if(!silent) showToast('â ï¸ Ø§ÙØªÙØª ØµÙØ§Ø­ÙØ© Ø§ÙØ±Ø¨Ø· â Ø£Ø¹Ø¯ Ø§ÙØ§ØªØµØ§Ù'); initGoogleDriveUI(); return false; }
      throw new Error((err.error && err.error.message) || 'Upload failed');
    }
    DB.s('lastDriveBackup', new Date().toISOString());
    if (!silent) showToast('âï¸ ØªÙ Ø±ÙØ¹ Ø§ÙÙØ³Ø®Ø© Ø¹ÙÙ Google Drive!');
    renderLastBackupInfo(); return true;
  } catch(err) { if (!silent) showToast('â ÙØ´Ù Ø§ÙØ±ÙØ¹: '+err.message); return false; }
}

function toggleAutoBackup(enabled) {
  DB.s('autoBackupEnabled', enabled);
  showToast(enabled ? 'â ØªÙ ØªÙØ¹ÙÙ Ø§ÙÙØ³Ø® Ø§ÙØªÙÙØ§Ø¦Ù' : 'â¹ï¸ ØªÙ Ø¥ÙÙØ§Ù Ø§ÙÙØ³Ø® Ø§ÙØªÙÙØ§Ø¦Ù');
}
...
async function checkAutoBackup(force) {
  ...
  if (_gdriveToken) {
    ...
    if (ok) showToast('âï¸ ØªÙ Ø§ÙÙØ³Ø® Ø§ÙØ§Ø­ØªÙØ§Ø·Ù Ø§ÙØªÙÙØ§Ø¦Ù Ø¹ÙÙ Drive');
  }
}
...
function logout() {
  if (!confirm('ÙÙ ØªØ±ÙØ¯ ØªØ³Ø¬ÙÙ Ø§ÙØ®Ø±ÙØ¬Ø')) return;
...
function applyTheme(theme) {
  ...
  if (btn) btn.textContent = theme === 'dark' ? 'âï¸' : 'ð';
}
...
// ââ MOBILE HELPERS ââââââââââââââââââââââââââ
...
function applyMobileUI() {
  ...
  // Manager topbar logout button â always shown after login, don't touch here
  // FAB
...
// ââ PRE-INIT: load passwords from Firebase before showing login ââ
async function preInitFirebase() {
  ...
  try {
    ...
    // Must sign in anonymously first â Firestore rules require auth
    await Promise.race([
...
function renderProducts() {
  ...
  if (currentBranch === 'wh') {
    ...
    if (grid) {
      ...
      msg.innerHTML = '<div style="font-size:48px;">ð­</div><h3 style="font-size:18px;font-weight:700;margin:12px 0 6px;">Ø§ÙÙØ®Ø²Ù Ø§ÙØ±Ø¦ÙØ³Ù</h3><p style="color:var(--text-muted);font-size:14px;">ÙØ§ ÙÙÙÙ Ø§ÙØ¨ÙØ¹ ÙÙ Ø§ÙÙØ®Ø²Ù Ø§ÙØ±Ø¦ÙØ³Ù<br>Ø§ÙØ±Ø¬Ø§Ø¡ Ø§ÙØªØ­ÙÙÙ ÙØ£Ø­Ø¯ Ø§ÙÙØ±ÙØ¹ Ø£ÙÙØ§Ù</p>';
      var btn = document.createElement('button');
      btn.className = 'btn btn-primary';
      btn.style.marginTop = '16px';
      btn.textContent = 'ð­ Ø§ÙØ°ÙØ§Ø¨ ÙÙÙØ®Ø²Ù';
  ...
  if (!items.length) {
    grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:60px;color:var(--text-muted);">' +
      (inv.length ? 'ÙÙ ÙÙØ¹Ø«Ø± Ø¹ÙÙ ÙØªØ§Ø¦Ø¬' : 'ð¦ ÙØ§ ØªÙØ¬Ø¯ ÙÙØªØ¬Ø§Øª â Ø§Ø³ØªÙØ±Ø¯ ÙÙÙ Excel ÙÙ ÙÙØ­Ø© Ø§ÙØ¥Ø¯Ø§Ø±Ø©') + '</div>';
    return;
  }
  grid.innerHTML = items.map(p => {
    ...
    return `<div class="product-card ${oos ? 'out-of-stock' : ''}" onclick="${oos ? '' : `addToCart('${p.code}')`}">
      ...
      <div class="product-price">${fmt(p.priceAfter)} Ø¬</div>
      ${p.priceBefore > p.priceAfter && p.priceBefore ? `<div class="product-old-price">${fmt(p.priceBefore)}</div>` : ''}
      <div class="product-stock">${oos ? 'â ÙÙØ¯' : `ð¦ ${p.qty}`}</div>
    </div>`;
  }).join('');
}
...
function addToCart(code) {
  ...
  if (ex) {
    if (ex.qty >= p.qty) { alert('ÙØ§ ÙÙØ¬Ø¯ ÙØ®Ø²ÙÙ ÙØ§ÙÙ'); return; }
    ex.qty++;
  } else {
...
function changeQty(code, d) {
  ...
  else if (p && item.qty > p.qty) { item.qty = p.qty; alert('ÙØ§ ÙÙØ¬Ø¯ ÙØ®Ø²ÙÙ ÙØ§ÙÙ'); }
  renderCart();
}
...
function clearCart() {
  if (cart.length && !confirm('ÙØ³Ø­ Ø§ÙÙØ§ØªÙØ±Ø©Ø')) return;
  cart = [];
...
function renderCart() {
  ...
  if (!cart.length) { el.innerHTML = '<div class="cart-empty">Ø£Ø¶Ù ÙÙØªØ¬Ø§Øª ÙÙÙØ§ØªÙØ±Ø©</div>'; updateCartUI(); return; }
  el.innerHTML = cart.map(i => {
    const modified = i.priceModified;
    const priceHtml = modified
      ? `<span style="text-decoration:line-through;color:#9ca3af;font-size:11px;">${fmt(i.originalPrice)} Ø¬</span> <span style="color:#dc2626;font-weight:700;">${fmt(i.price)} Ø¬</span> Ã ${i.qty}`
      : `${fmt(i.price)} Ø¬ Ã ${i.qty}`;
    return `<div class="cart-item" style="${modified?'background:#fff5f5;border-right:3px solid #ef4444;':''}">
      ...
      <div class="qty-ctrl">
        <button class="qty-btn" onclick="changeQty('${i.code}',-1)">â</button>
        <span class="qty-num">${i.qty}</span>
        <button class="qty-btn" onclick="changeQty('${i.code}',1)">+</button>
      </div>
      <button onclick="editItemPrice('${i.code}')" style="background:${modified?'#fef3c7':'#f3f4f6'};border:none;border-radius:5px;padding:3px 7px;cursor:pointer;font-size:12px;" title="ØªØ¹Ø¯ÙÙ Ø§ÙØ³Ø¹Ø±">âï¸</button>
      <div class="item-total">${fmt(i.price*i.qty)}</div>
      <button class="delete-item" onclick="removeFromCart('${i.code}')">â</button>
    </div>`;
  }).join('');
  updateCartUI();
}
...
function updateCartUI() {
  ...
  document.getElementById('cartSubtotal').textContent = fmt(sub) + ' Ø¬';
  document.getElementById('cartTotal').textContent    = fmt(total) + ' Ø¬';
  ...
  if (adminDisc > 0 && adRow && adEl) {
    ...
    adEl.textContent = '-' + fmt(adminDisc) + ' Ø¬';
  } else if (adRow) {
    ...
    aprEl.innerHTML = (cart._appliedPromos || []).map(p => `
      <div class="cart-row" style="color:#7c3aed; background:#f5f3ff; padding:5px 8px; border-radius:6px; margin:2px 0; align-items:center;">
        <span style="display:flex; align-items:center; gap:4px;">
          ð·ï¸ ${p.name}
          <button onclick="removeAppliedPromo('${p.id}')" style="background:none; border:none; cursor:pointer; color:#ef4444; font-size:12px; padding:0 2px;" title="Ø¥Ø²Ø§ÙØ© Ø§ÙØ¹Ø±Ø¶">â</button>
        </span>
        <span>-${fmt(p.discAmt)} Ø¬</span>
      </div>`).join('');
  }
  ...
  if (payBtn) {
    if (hasModified) {
      payBtn.textContent = 'ð¤ Ø¥Ø±Ø³Ø§Ù ÙÙÙÙØ§ÙÙØ©';
      payBtn.style.background = '#d97706';
      payBtn.onclick = sendForApproval;
    } else {
      payBtn.textContent = 'ð³ Ø¯ÙØ¹';
...
function openPayment() {
  if (!cart.length) { alert('Ø§ÙÙØ§ØªÙØ±Ø© ÙØ§Ø±ØºØ©'); return; }
  const { total } = cartTotals();
  document.getElementById('modalTotal').textContent = fmt(total) + ' Ø¬';
  document.getElementById('paidAmount').value = '';
  document.getElementById('changeAmt').textContent = '0.00 Ø¬';
...
function calcChange() {
  ...
  el.textContent = fmt(Math.max(0, change)) + ' Ø¬';
...
function completeSale() {
  ...
  if (payMethod === 'cash') {
    ...
    if (paid < total) { alert('Ø§ÙÙØ¨ÙØº Ø§ÙÙØ¯ÙÙØ¹ Ø£ÙÙ ÙÙ Ø§ÙØ¥Ø¬ÙØ§ÙÙ'); return; }
    change = paid - total;
  }
  ...
  const sale = {
    ...
    cashier: currentUser === 'admin' ? 'ÙØ¯ÙØ±' : 'ÙØ§Ø´ÙØ±',
    salesperson,
    items: cart.map(i => ({...i})),
  ...
  addAuditLog('sale.complete', `ÙØ§ØªÙØ±Ø© #${String(sale.id||'').slice(-6)} â ${fmt(sale.total)} Ø¬ â ${sale.items.length} ØµÙÙ`, sale.branchId);
...
function showReceipt(sale) {
  ...
  const lines = sale.items.map(i =>
    `<div style="display:flex;justify-content:space-between;">
      <span>${i.name} Ã ${i.qty}</span><span>${fmt(i.price*i.qty)} Ø¬</span>
    </div>`).join('');
  document.getElementById('receiptContent').innerHTML = `
    <div style="text-align:center;margin-bottom:10px;">
      ...
      <div style="font-size:13px;font-weight:700;margin-top:4px;">ÙØ§ØªÙØ±Ø© ÙØ¨ÙØ¹Ø§Øª</div>
      <div style="font-size:11px;color:gray;">${new Date(sale.date).toLocaleString('ar-EG')}</div>
      <div style="font-size:11px;color:gray;">Ø±ÙÙ: ${String(sale.id).slice(-8)}</div>
      ${sale.salesperson ? `<div style="font-size:11px;color:gray;">Ø§ÙØ¨Ø§Ø¦Ø¹: ${sale.salesperson}</div>` : ''}
    </div>
    <hr style="border:1px dashed #ccc;margin:8px 0;">
    ${lines}
    <hr style="border:1px dashed #ccc;margin:8px 0;">
    <div style="display:flex;justify-content:space-between;"><span>Ø§ÙÙØ¬ÙÙØ¹</span><span>${fmt(sale.sub)} Ø¬</span></div>
    ${sale.disc > 0 ? `<div style="display:flex;justify-content:space-between;color:green;"><span>Ø®ØµÙ</span><span>-${fmt(sale.disc)} Ø¬</span></div>` : ''}
    <div style="display:flex;justify-content:space-between;font-weight:700;font-size:15px;margin-top:4px;border-top:1px solid #ccc;padding-top:4px;">
      <span>Ø§ÙØ¥Ø¬ÙØ§ÙÙ</span><span>${fmt(sale.total)} Ø¬</span>
    </div>
    ${sale.payMethod === 'cash' ? `
    <div style="display:flex;justify-content:space-between;color:#2563eb;margin-top:4px;"><span>ÙØ¯ÙÙØ¹</span><span>${fmt(sale.paid)} Ø¬</span></div>
    <div style="display:flex;justify-content:space-between;color:green;font-weight:700;"><span>Ø§ÙØ¨Ø§ÙÙ</span><span>${fmt(sale.change)} Ø¬</span></div>
    ` : '<div style="text-align:center;color:#2563eb;margin-top:6px;">ð³ Ø¯ÙØ¹ Ø¨Ø§ÙÙØ§Ø±Øª</div>'}
    <div style="text-align:center;margin-top:12px;font-size:12px;color:gray;">Ø´ÙØ±Ø§Ù ÙØªØ¹Ø§ÙÙÙÙ ÙØ¹ÙØ§ ð</div>`;
...
function showPage(page) {
  ...
  const titles = { dashboard:'Ø§ÙØ±Ø¦ÙØ³ÙØ©', inventory:'Ø¥Ø¯Ø§Ø±Ø© Ø§ÙÙØ®Ø²ÙÙ', sales:'Ø³Ø¬Ù Ø§ÙÙØ¨ÙØ¹Ø§Øª', suspended:'ÙÙØ§ØªÙØ± ÙØ¹ÙÙØ©', reports:'Ø§ÙØªÙØ§Ø±ÙØ±', customized:'ØªÙØ§Ø±ÙØ± ÙØ®ØµØµØ©', home:'Ø§ÙØ±Ø¦ÙØ³ÙØ©', warehouse:'Ø§ÙÙØ®Ø²Ù Ø§ÙØ±Ø¦ÙØ³Ù', settings:'Ø§ÙØ¥Ø¹Ø¯Ø§Ø¯Ø§Øª', customers:'Ø§ÙØ¹ÙÙØ§Ø¡', purchases:'Ø§ÙÙØ´ØªØ±ÙØ§Øª', transfers:'التحويلات', promos:'العروض', hr:'Ø§ÙÙÙØ§Ø±Ø¯ Ø§ÙØ¨Ø´Ø±ÙØ©', expenses:'Ø§ÙÙØµØ§Ø±ÙÙ', audit:'Ø³Ø¬Ù Ø§ÙØªØºÙÙØ±Ø§Øª', accounting:'Ø§ÙÙØ­Ø§Ø³Ø¨Ø© Ø§ÙØ±Ø³ÙÙØ©' };
...
function deltaBadge(curr, prev, isPercent) {
  if (prev === 0) return '<span style="color:#94a3b8; font-size:11px;">ÙØ§ ÙÙØ¬Ø¯ ÙÙØ§Ø±ÙØ©</span>';
  ...
  return `<span style="color:${color}; font-size:12px; font-weight:700;">${arrow} ${Math.abs(pct)}%</span>
          <span style="color:#94a3b8; font-size:10px; margin-right:4px;">vs Ø§ÙØ³Ø§Ø¨Ù</span>`;
}
...
function buildDashboard() {
  ...
  if (labelEl) {
    labelEl.textContent = `${rangeStart.toLocaleDateString('ar-EG',{day:'numeric',month:'short'})} â ${now.toLocaleDateString('ar-EG',{day:'numeric',month:'short',year:'numeric'})}`;
  }
  ...
  document.getElementById('sd-today').textContent    = fmt(todaySales.reduce((s,x) => s+x.total,0)) + ' Ø¬';
  document.getElementById('sd-today-c').textContent  = todaySales.length + ' ÙØ§ØªÙØ±Ø©';
  document.getElementById('sd-month').textContent    = fmt(rangeSales.reduce((s,x) => s+x.total,0)) + ' Ø¬';
  document.getElementById('sd-month-c').textContent  = rangeSales.length + ' ÙØ§ØªÙØ±Ø©';
  document.getElementById('sd-profit').textContent   = fmt(rangeProfit) + ' Ø¬';
  ...
  // ââ Month vs Prev Month comparison cards ââ
  ...
  cmpEl('cmp-rev',    thisRev,    prevRev,    ' Ø¬');
  cmpEl('cmp-profit', monthProfit, prevMonthProfit, ' Ø¬');
  cmpEl('cmp-orders', thisOrders, prevOrders, ' Ù');
  cmpEl('cmp-atv',    thisATV,    prevATV,    ' Ø¬');
  ...
  // ââ Weekly chart (last 7 days) ââ
  ...
  chartWeekly = new Chart(document.getElementById('chartWeekly'), {
    type:'bar',
    data:{ labels:days.map(d=>d.toLocaleDateString('ar-EG',{weekday:'short',day:'numeric'})), datasets:[{data:weeklyData,backgroundColor:'rgba(200,25,60,.75)',borderRadius:6,label:'Ø§ÙÙØ¨ÙØ¹Ø§Øª'}] },
    options:{ plugins:{legend:{display:false}}, scales:{y:{beginAtZero:true,ticks:{callback:v=>v>0?fmt(v):'0'}}}, responsive:true }
  });

  // ââ Top products doughnut ââ
  const ps = {};
  ...
  // ââ 6-month revenue trend line chart ââ
  ...
  chartTrend = new Chart(document.getElementById('chartTrend'), {
    ...
    data:{
      ...
      datasets:[{ data:trendData, borderColor:'#1a5faf', backgroundColor:'rgba(26,95,175,.08)', tension:.4, fill:true, pointBackgroundColor:'#1a5faf', pointRadius:5, label:'Ø§ÙØ¥ÙØ±Ø§Ø¯Ø§Øª' }]
    },
    options:{ plugins:{legend:{display:false}}, scales:{y:{beginAtZero:true,ticks:{callback:v=>fmt(v)}}}, responsive:true }
  });

  // ââ Branch comparison horizontal bar chart ââ
  const branches = getBranches();
  ...
  chartBranches = new Chart(document.getElementById('chartBranches'), {
    ...
    data:{
      labels: branchLabels,
      datasets:[{ data:branchRevs, backgroundColor:['#1a5faf','#F47920','#d97706','#7c3aed'], borderRadius:6, label:'Ø§ÙØ¥ÙØ±Ø§Ø¯Ø§Øª' }]
    },
    options:{
      indexAxis:'y',
  ...
  if (execTargetSub) { execTargetSub.textContent = totalTarget>0 ? `ÙØ¯Ù: ${fmt(totalTarget)} Ø¬` : 'ÙÙ ÙÙØ­Ø¯Ø¯ ÙØ¯Ù'; }
  if (execTurnover)  execTurnover.textContent = turnover.toFixed(1)+'x';
  if (execCoverage)  execCoverage.textContent = coverage;
  if (execInvVal)    execInvVal.textContent = fmt(invValue)+' Ø¬';

  // Low stock list
  const allAlert = [...out, ...low].sort((a,b) => a.qty-b.qty);
  const dashEl = document.getElementById('dashLowStock');
  if (!allAlert.length) {
    dashEl.innerHTML = '<div class="text-center text-muted" style="padding:20px;">ÙØ§ ØªÙØ¬Ø¯ ÙÙØªØ¬Ø§Øª ØªØ­ØªØ§Ø¬ ØªØ¬Ø¯ÙØ¯ ð</div>';
  } else {
    dashEl.innerHTML = `<table style="width:100%;"><thead><tr><th>Ø§ÙÙÙØ¯</th><th>Ø§ÙØ§Ø³Ù</th><th>Ø§ÙÙÙÙØ© Ø§ÙØ­Ø§ÙÙØ©</th><th>Ø§ÙØ­Ø§ÙØ©</th></tr></thead><tbody>
      ${allAlert.map(p=>`<tr><td>${p.code}</td><td>${p.name}</td><td><strong>${p.qty}</strong></td>
        <td><span class="badge ${p.qty<=0?'badge-danger':'badge-warning'}">${p.qty<=0?'ÙÙØ¯ Ø§ÙÙØ®Ø²ÙÙ':'ÙØ®Ø²ÙÙ ÙÙØ®ÙØ¶'}</span></td></tr>`).join('')}
    </tbody></table>`;
  }
}
...
function renderInventory() {
  ...
  if (_cf) { const _cv=_cf.value; const _cats=[...new Set(inv.map(x=>x.category).filter(Boolean))].sort(); _cf.innerHTML='<option value="">ÙÙ Ø§ÙÙØ¦Ø§Øª</option>'+_cats.map(c=>`<option value="${c}" ${c===_cv?'selected':''}>${c}</option>`).join(''); _cf.value=_cv; }
  if (_ff) { const _fv=_ff.value; const _fams=[...new Set(inv.map(x=>x.family).filter(Boolean))].sort(); _ff.innerHTML='<option value="">ÙÙ Ø§ÙÙØ¬ÙÙØ¹Ø§Øª</option>'+_fams.map(f=>`<option value="${f}" ${f===_fv?'selected':''}>${f}</option>`).join(''); _ff.value=_fv; }

  document.getElementById('inventoryBody').innerHTML = !items.length
    ? '<tr><td colspan="10" class="text-center text-muted" style="padding:40px;">ÙØ§ ØªÙØ¬Ø¯ ÙÙØªØ¬Ø§Øª</td></tr>'
    : items.map(p => `<tr>
        <td><strong>${p.code}</strong></td>
        <td>${p.name}</td>
        <td><span style="background:#eff6ff;color:#1d4ed8;padding:2px 8px;border-radius:12px;font-size:11px;">${p.category||'â'}</span></td>
        <td><span style="background:#f3f4f6;color:#374151;padding:2px 8px;border-radius:12px;font-size:11px;">${p.family||'â'}</span></td>
        <td>${fmt(p.cost||0)}</td>
        ...
        <td><span class="badge ${p.qty<=0?'badge-danger':p.qty<=thresh?'badge-warning':'badge-success'}">
          ${p.qty<=0?'ÙÙØ¯':p.qty<=thresh?'ÙÙØ®ÙØ¶':'ÙØªÙÙØ±'}
        </span></td>
        <td>
          <button class="btn btn-gray btn-sm" onclick="editProduct('${p.code}')" style="margin-left:4px;">âï¸</button>
          <button class="btn btn-danger btn-sm" onclick="deleteProduct('${p.code}')">ðï¸</button>
        </td>
      </tr>`).join('');
}
...
function openProductModal(p) {
  document.getElementById('pmTitle').textContent = p ? 'ØªØ¹Ø¯ÙÙ ÙÙØªØ¬' : 'Ø¥Ø¶Ø§ÙØ© ÙÙØªØ¬ Ø¬Ø¯ÙØ¯';
...
function saveProduct() {
  ...
  if (!code || !name || isNaN(priceAfter)) { alert('Ø§ÙÙÙØ¯ ÙØ§ÙØ§Ø³Ù ÙØ§ÙØ³Ø¹Ø± ÙØ·ÙÙØ¨ÙÙ'); return; }
  ...
  if (editCode) {
    ...
    if (oldProd && oldProd.priceAfter !== prod.priceAfter) {
      addAuditLog('price.change', `${prod.name}: Ø³Ø¹Ø± ${fmt(oldProd.priceAfter)} â ${fmt(prod.priceAfter)} Ø¬`, null);
    } else {
      addAuditLog('inv.edit', `ØªØ¹Ø¯ÙÙ: ${prod.name} (${prod.code}) â ÙÙÙØ©: ${prod.qty}`, null);
    }
  } else {
    if (inv.find(x => x.code === code)) { alert('ÙØ°Ø§ Ø§ÙÙÙØ¯ ÙÙØ¬ÙØ¯ ÙØ³Ø¨ÙØ§Ù'); return; }
    inv.push(prod);
    addAuditLog('inv.add', `Ø¥Ø¶Ø§ÙØ©: ${prod.name} (${prod.code}) â Ø³Ø¹Ø±: ${fmt(prod.priceAfter)} Ø¬`, null);
  }
  setInv(inv);
...
function deleteProduct(code) {
  if (!confirm('Ø­Ø°Ù ÙØ°Ø§ Ø§ÙÙÙØªØ¬Ø')) return;
  const prod = getInv().find(x => x.code === code);
  setInv(getInv().filter(x => x.code !== code));
  if (prod) addAuditLog('inv.delete', `Ø­Ø°Ù: ${prod.name} (${prod.code})`, null);
  renderInventory();
}
...
function importExcel(e) {
  ...
  reader.onload = ev => {
    ...
    rows.forEach((row, i) => {
      ...
      const code = String(g('code','ÙÙØ¯','Ø§ÙÙÙØ¯')||'').trim();
      const name = String(g('name','Ø§Ø³Ù','Ø§ÙØ§Ø³Ù','product name','product','item','item name')||'').trim();
      if (!code||!name) { errors.push(`Ø³Ø·Ø± ${i+2}: ÙÙØ¯ Ø£Ù Ø§Ø³Ù ÙÙÙÙØ¯`); return; }

      const priceAfter = parseFloat(g('price after','price_after','priceafter','Ø§ÙØ³Ø¹Ø± Ø¨Ø¹Ø¯','Ø³Ø¹Ø± Ø¨Ø¹Ø¯','price','Ø§ÙØ³Ø¹Ø±')||0) || 0;

      const prod = {
        code, name,
        cost:        parseFloat(g('cost','Ø§ÙØªÙÙÙØ©','ØªÙÙÙØ©','buy price','buying price','purchase price','Ø³Ø¹Ø± Ø§ÙØ´Ø±Ø§Ø¡')||0)||0,
        priceBefore: parseFloat(g('price before','price_before','pricebefore','Ø§ÙØ³Ø¹Ø± ÙØ¨Ù','Ø³Ø¹Ø± ÙØ¨Ù','old price')||0)||0,
        priceAfter,
        qty:      parseInt(g('qty','quantity','Ø§ÙÙÙÙØ©','ÙÙÙØ©','stock','ÙØ®Ø²ÙÙ')||0)||0,
        category: String(g('category','Ø§ÙÙØ¦Ø©','ÙØ¦Ø©','ÙØ§ØªÙØ¬ÙØ±Ù','ÙØ§ØªÙØ¬ÙØ±Ù','')||'').trim(),
        family:   String(g('family','Ø§ÙÙØ¬ÙÙØ¹Ø©','ÙØ¬ÙÙØ¹Ø©','ÙØ§ÙÙÙÙ','ÙØ§ÙÙÙÙ','')||'').trim()
      };
    ...
    const msg = `â ØªÙ Ø§ÙØ§Ø³ØªÙØ±Ø§Ø¯: ${added} ÙÙØªØ¬ Ø¬Ø¯ÙØ¯ Â· ${updated} ØªÙ ØªØ­Ø¯ÙØ«Ù${errors.length ? `<br>â ï¸ ${errors.slice(0,3).join(' | ')}` : ''}`;
...
function exportInventoryExcel() {
  const data = getInv().map(p => ({
    'Ø§ÙÙÙØ¯':p.code,'Ø§ÙØ§Ø³Ù':p.name,'Ø§ÙØªÙÙÙØ©':p.cost||0,
    'Ø§ÙØ³Ø¹Ø± ÙØ¨Ù':p.priceBefore||0,'Ø§ÙØ³Ø¹Ø± Ø¨Ø¹Ø¯':p.priceAfter,'Ø§ÙÙÙÙØ©':p.qty
  }));
  const ws = XLSX.utils.json_to_sheet(data);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Ø§ÙÙØ®Ø²ÙÙ');
...
function renderSales() {
  ...
  if (sbf && sbf.options.length === 0) {
    var opt0 = document.createElement('option'); opt0.value='all'; opt0.textContent='ð¬ ÙÙ Ø§ÙÙØ±ÙØ¹'; sbf.appendChild(opt0);
    var bnames = getBranches();
  ...
  document.getElementById('salesBody').innerHTML = !sales.length
    ? '<tr><td colspan="10" class="text-center text-muted" style="padding:40px;">ÙØ§ ØªÙØ¬Ø¯ ÙØ¨ÙØ¹Ø§Øª</td></tr>'
    : sales.map(s => `<tr class="${s.isReturn ? 'return-row' : ''}">
        <td>${s.isReturn ? `<span class="badge-return">ÙØ±ØªØ¬Ø¹ #${String(s.id).slice(-6)}</span>` : `<span class="badge badge-info">#${String(s.id).slice(-6)}</span>`}</td>
        ...
        <td>${fmt(s.sub)} Ø¬</td>
        <td>${s.disc>0?fmt(s.disc)+' Ø¬':'-'}</td>
        <td><strong>${fmt(s.total)} Ø¬</strong></td>
        <td>${s.payMethod==='cash'?'ðµ ÙÙØ¯Ù':'ð³ ÙØ§Ø±Øª'}</td>
        <td><button class="btn btn-gray btn-sm" onclick="viewSale(${s.id})">ðï¸ Ø¹Ø±Ø¶</button></td>
      </tr>`).join('');
}
...
function viewSale(id) {
  ...
  document.getElementById('saleDetailContent').innerHTML = `
    <div class="grid-2" style="font-size:13px;margin-bottom:14px;gap:8px;">
      <div><strong>Ø±ÙÙ Ø§ÙÙØ§ØªÙØ±Ø©:</strong> ${String(sale.id).slice(-8)}</div>
      <div><strong>Ø§ÙØªØ§Ø±ÙØ®:</strong> ${new Date(sale.date).toLocaleString('ar-EG')}</div>
      <div><strong>Ø§ÙØ¨Ø§Ø¦Ø¹:</strong> ${sale.salesperson || sale.cashier || '-'}</div>
      <div><strong>Ø§ÙØ¯ÙØ¹:</strong> ${sale.payMethod==='cash'?'ðµ ÙÙØ¯Ù':'ð³ ÙØ§Ø±Øª'}</div>
    </div>
    <div class="table-wrap">
      <table style="font-size:13px;">
        <thead><tr><th>Ø§ÙÙÙØªØ¬</th><th>Ø§ÙÙÙÙØ©</th><th>Ø§ÙØ³Ø¹Ø±</th><th>Ø§ÙØ¥Ø¬ÙØ§ÙÙ</th></tr></thead>
        <tbody>${sale.items.map(i=>`<tr><td>${i.name}</td><td>${i.qty}</td><td>${fmt(i.price)} Ø¬</td><td><strong>${fmt(i.price*i.qty)} Ø¬</strong></td></tr>`).join('')}</tbody>
      </table>
    </div>
    <div style="background:#f8fafc;border-radius:8px;padding:12px;margin-top:12px;font-size:14px;">
      <div class="flex justify-between"><span>Ø§ÙÙØ¬ÙÙØ¹</span><span>${fmt(sale.sub)} Ø¬</span></div>
      ${sale.disc>0?`<div class="flex justify-between" style="color:green;"><span>Ø®ØµÙ</span><span>-${fmt(sale.disc)} Ø¬</span></div>`:''}
      <div class="flex justify-between font-bold" style="font-size:16px;margin-top:6px;border-top:1px solid var(--border);padding-top:6px;"><span>Ø§ÙØ¥Ø¬ÙØ§ÙÙ</span><span>${fmt(sale.total)} Ø¬</span></div>
      ${sale.payMethod==='cash'?`<div class="flex justify-between" style="color:var(--primary);"><span>ÙØ¯ÙÙØ¹</span><span>${fmt(sale.paid)} Ø¬</span></div><div class="flex justify-between" style="color:green;font-weight:700;"><span>Ø§ÙØ¨Ø§ÙÙ</span><span>${fmt(sale.change)} Ø¬</span></div>`:''}
    </div>`;
...
function populateSellerFilter(selId) {
  ...
  const cur = sel.value;
  const people = getSalespeople();
  // Include 'ØºÙØ± ÙØ­Ø¯Ø¯' if any sale lacks salesperson
  const all = getSales().map(s => s.salesperson || '');
  const hasBlank = all.some(x => !x);
  sel.innerHTML = '<option value="">ð¤ ÙÙ Ø§ÙØ¨Ø§Ø¦Ø¹ÙÙ</option>' +
    people.map(n=>`<option value="${n}">${n}</option>`).join('') +
    (hasBlank ? '<option value="__none__">ØºÙØ± ÙØ­Ø¯Ø¯</option>' : '');
...
function renderCmpRow(containerId, cur, prev, labels) {
  ...
  el.innerHTML = labels.map(({key, label, unit}) => {
    ...
    const displayVal = unit === 'pct' ? c.toFixed(1)+'%' : fmt(c)+' Ø¬';
    return `<div class="cmp-card">
      <div class="cmp-label">${label}</div>
      <div class="cmp-val">${displayVal}</div>
      <div class="cmp-delta ${cls}">${arrow} ${Math.abs(pct).toFixed(1)}% ÙÙØ§Ø±ÙØ© Ø¨Ø§ÙÙØªØ±Ø© Ø§ÙØ³Ø§Ø¨ÙØ©</div>
    </div>`;
  }).join('');
}
...
function buildSalesReport() {
  ...
  document.getElementById('rs-total').textContent = fmt(sub)+' Ø¬';
  document.getElementById('rs-net').textContent   = fmt(net)+' Ø¬';
  document.getElementById('rs-count').textContent = sales.length;
  document.getElementById('rs-avg').textContent   = fmt(sales.length ? net/sales.length : 0)+' Ø¬';
  ...
  if (period !== 'custom') {
    ...
    renderCmpRow('salesCmpRow',
      ...
      [
        { key:'net',   label:'Ø§ÙØ¥ÙØ±Ø§Ø¯Ø§Øª',     unit:'amount' },
        { key:'count', label:'Ø§ÙÙÙØ§ØªÙØ±',       unit:'amount' },
        { key:'avg',   label:'ÙØªÙØ³Ø· Ø§ÙÙØ§ØªÙØ±Ø©', unit:'amount' }
      ]
    );
  } else {
  ...
  chartRptSales = new Chart(document.getElementById('chartRptSales'), {
    type:'line',
    data:{ labels:days, datasets:[{data:days.map(d=>dayMap[d]),borderColor:'#1a5faf',backgroundColor:'rgba(26,95,175,.1)',fill:true,tension:.3,label:'Ø§ÙÙØ¨ÙØ¹Ø§Øª'}] },
  ...
  document.getElementById('rs-breakdown').innerHTML = !sorted.length
    ? '<tr><td colspan="3" class="text-center text-muted" style="padding:16px;">ÙØ§ ØªÙØ¬Ø¯ ÙØ¨ÙØ¹Ø§Øª</td></tr>'
    : sorted.map(d=>`<tr><td>${d.name}</td><td>${d.qty}</td><td><strong>${fmt(d.rev)} Ø¬</strong></td></tr>`).join('');
}
...
function buildInventoryReport() {
  ...
  document.getElementById('ri-cost').textContent  = fmt(costVal)+' Ø¬';
  document.getElementById('ri-sell').textContent  = fmt(sellVal)+' Ø¬';
  ...
  document.getElementById('ri-body').innerHTML = sorted.map(p => {
    ...
    return `<tr>
      ...
      <td>${fmt(stockVal)} Ø¬</td>
      <td style="color:${profit>=0?'var(--success)':'var(--danger)'};">${fmt(profit)} Ø¬</td>
      <td><span class="badge ${p.qty<=0?'badge-danger':p.qty<=thresh?'badge-warning':'badge-success'}">
        ${p.qty<=0?'ÙÙØ¯':p.qty<=thresh?'ÙÙØ®ÙØ¶':'ÙØªÙÙØ±'}
      </span></td>
    </tr>`;
...
function buildProfitReport() {
  ...
  document.getElementById('rp-revenue').textContent   = fmt(revenue)+' Ø¬';
  document.getElementById('rp-cost').textContent      = fmt(totalCost)+' Ø¬';
  document.getElementById('rp-profit').textContent    = fmt(profit)+' Ø¬';
  document.getElementById('rp-margin').textContent    = margin.toFixed(1)+'%';
  document.getElementById('rp-discounts').textContent = fmt(totalDisc)+' Ø¬';
  ...
  if (period !== 'custom') {
    ...
    renderCmpRow('profitCmpRow',
      ...
      [
        { key:'revenue', label:'Ø§ÙØ¥ÙØ±Ø§Ø¯Ø§Øª',  unit:'amount' },
        { key:'profit',  label:'ØµØ§ÙÙ Ø§ÙØ±Ø¨Ø­', unit:'amount' },
        { key:'margin',  label:'ÙØ§ÙØ´ Ø§ÙØ±Ø¨Ø­', unit:'pct'    }
      ]
    );
  } else {
  ...
  chartProfit = new Chart(document.getElementById('chartProfit'), {
    type:'bar',
    data:{
      labels:['Ø§ÙØ¥ÙØ±Ø§Ø¯Ø§Øª','Ø§ÙØªÙÙÙØ©','ØµØ§ÙÙ Ø§ÙØ±Ø¨Ø­'],
  ...
  document.getElementById('rp-breakdown').innerHTML = !sorted2.length
    ? '<tr><td colspan="6" class="text-center text-muted" style="padding:16px;">ÙØ§ ØªÙØ¬Ø¯ Ø¨ÙØ§ÙØ§Øª</td></tr>'
    : sorted2.map(d => {
        const p=d.rev-d.cost, m=d.rev?p/d.rev*100:0;
        return `<tr><td>${d.name}</td><td>${d.qty}</td><td>${fmt(d.rev)} Ø¬</td><td>${fmt(d.cost)} Ø¬</td>
          <td style="color:${p>=0?'var(--success)':'var(--danger)'};font-weight:700;">${fmt(p)} Ø¬</td>
...
function buildKPIReport() {
  ...
  document.getElementById('kpi-atv').textContent      = fmt(atv)+' Ø¬';
  document.getElementById('kpi-upt').textContent      = upt.toFixed(2);
  document.getElementById('kpi-gm').textContent       = gm.toFixed(1)+'%';
  document.getElementById('kpi-turnover').textContent = turnover.toFixed(2)+'Ã';
  ...
  document.getElementById('tv-cogs').textContent      = fmt(cogs)+' Ø¬';
  document.getElementById('tv-inv').textContent       = fmt(invValue)+' Ø¬';
  document.getElementById('tv-rate').textContent      = turnover.toFixed(2)+'Ã';
...
function buildSellersReport() {
  ...
  const bd = {};
  sales.forEach(s => {
    const name = s.salesperson || 'ØºÙØ± ÙØ­Ø¯Ø¯';
  ...
  document.getElementById('sellersStatsGrid').innerHTML = rows.map(r => `
    <div class="stat-card">
      <div class="stat-label">ð¤ ${r.name}</div>
      <div class="stat-value" style="font-size:20px;">${fmt(r.revenue)} Ø¬</div>
      <div class="stat-sub">${r.count} ÙØ§ØªÙØ±Ø© Â· ${r.units} ÙØ­Ø¯Ø©</div>
    </div>`).join('');

  document.getElementById('sellers-body').innerHTML = !rows.length
    ? '<tr><td colspan="7" class="text-center text-muted" style="padding:20px;">ÙØ§ ØªÙØ¬Ø¯ Ø¨ÙØ§ÙØ§Øª</td></tr>'
    : rows.map(r => {
        ...
        return `<tr>
          ...
          <td>${r.count}</td>
          <td>${r.units}</td>
          <td>${fmt(r.revenue)} Ø¬</td>
          <td>${fmt(atv)} Ø¬</td>
          <td>${upt.toFixed(2)}</td>
          <td style="color:var(--warning);">${fmt(r.disc)} Ø¬</td>
        </tr>`;
      }).join('');
  ...
  if (rows.length) {
    chartSellers = new Chart(document.getElementById('chartSellers'), {
      ...
      data:{
        ...
        datasets:[
          {label:'Ø§ÙØ¥ÙØ±Ø§Ø¯Ø§Øª',data:rows.map(r=>r.revenue),backgroundColor:'rgba(200,25,60,.8)',borderRadius:6},
          {label:'Ø§ÙØ®ØµÙÙØ§Øª',data:rows.map(r=>r.disc),backgroundColor:'rgba(217,119,6,.6)',borderRadius:6}
        ]
      },
...
function renderLowStockPanel() {
  ...
  if (!items.length) {
    el.innerHTML = '<div style="text-align:center;padding:24px;color:var(--text-muted);">ÙÙ Ø§ÙÙÙØªØ¬Ø§Øª ÙØªÙÙØ±Ø©</div>';
    return;
  }
  el.innerHTML = items.map(p => `
    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 4px;border-bottom:1px solid var(--border);">
      <div>
        ...
        <div style="font-size:11px;color:var(--text-muted);">ÙÙØ¯: ${p.code}</div>
      </div>
      <span class="badge ${p.qty<=0?'badge-danger':'badge-warning'}" style="font-size:12px;padding:3px 10px;">
        ${p.qty<=0?'ÙÙØ¯':'ÙØªØ¨ÙÙ '+p.qty}
      </span>
    </div>`).join('');
}
...
function exportReorderList() {
  ...
  if (!items.length) { alert('ÙØ§ ØªÙØ¬Ø¯ ÙÙØªØ¬Ø§Øª ØªØ­ØªØ§Ø¬ ØªØ¬Ø¯ÙØ¯'); return; }
  const data = items.map(p => ({
    'Ø§ÙÙÙØ¯': p.code, 'Ø§ÙØ§Ø³Ù': p.name,
    'Ø§ÙÙÙÙØ© Ø§ÙØ­Ø§ÙÙØ©': p.qty, 'Ø­Ø¯ Ø§ÙØªÙØ¨ÙÙ': thresh,
    'Ø§ÙØ­Ø§ÙØ©': p.qty<=0?'ÙÙØ¯ Ø§ÙÙØ®Ø²ÙÙ':'ÙØ®Ø²ÙÙ ÙÙØ®ÙØ¶'
  }));
  const ws = XLSX.utils.json_to_sheet(data);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'ÙØ§Ø¦ÙØ© Ø§ÙØ·ÙØ¨');
...
function buildHeatmap(sales) {
  ...
  if (!sales || !sales.length) {
    wrap.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ Ø¨ÙØ§ÙØ§Øª ÙÙ ÙØ°Ù Ø§ÙÙØªØ±Ø©</div>';
    return;
  }

  const dayNames = ['Ø§ÙØ£Ø­Ø¯','Ø§ÙØ§Ø«ÙÙÙ','Ø§ÙØ«ÙØ§Ø«Ø§Ø¡','Ø§ÙØ£Ø±Ø¨Ø¹Ø§Ø¡','Ø§ÙØ®ÙÙØ³','Ø§ÙØ¬ÙØ¹Ø©','Ø§ÙØ³Ø¨Øª'];
  ...
  let html = '<table class="heatmap-table"><thead><tr><th>Ø§ÙÙÙÙ</th>';
  ...
  for (let d = 0; d < 7; d++) {
    ...
    hours.forEach(h => {
      ...
      html += `<td style="background:${bg};" title="${dayNames[d]} ${h}:00 â ${fmt(v)} Ø¬">${label}</td>`;
    });
    html += '</tr>';
  }
  html += '</tbody></table>';
...
function exportBackup() {
  ...
  showMsg('sBackupMsg', 'ØªÙ ØªØµØ¯ÙØ± Ø§ÙÙØ³Ø®Ø© Ø§ÙØ§Ø­ØªÙØ§Ø·ÙØ© Ø¨ÙØ¬Ø§Ø­');
  renderLastBackupInfo();
}

function importBackup(e) {
  const file = e.target.files[0]; if (!file) return;
  if (!confirm('Ø§Ø³ØªØ¹Ø§Ø¯Ø© Ø§ÙÙØ³Ø®Ø© Ø§ÙØ§Ø­ØªÙØ§Ø·ÙØ© Ø³ØªØ³ØªØ¨Ø¯Ù Ø§ÙØ¨ÙØ§ÙØ§Øª Ø§ÙØ­Ø§ÙÙØ©. ÙÙ ØªØ±ÙØ¯ Ø§ÙÙØªØ§Ø¨Ø¹Ø©Ø')) {
  ...
  reader.onload = ev => {
    try {
      ...
      if (!data.version || !data.inv) { throw new Error('ÙÙÙ ØºÙØ± ØµØ§ÙØ­'); }
      ...
      showMsg('sBackupMsg', 'ØªÙ Ø§ÙØ§Ø³ØªØ¹Ø§Ø¯Ø© â ' + (data.inv?.length||0) + ' ÙÙØªØ¬ Â· ' + (data.sales?.length||0) + ' ÙØ§ØªÙØ±Ø©');
      buildDashboard();
    } catch(err) {
      showMsg('sBackupMsg', 'Ø®Ø·Ø£ ÙÙ ÙØ±Ø§Ø¡Ø© Ø§ÙÙÙÙ: ' + err.message, 'danger');
    }
    e.target.value = '';
  };
...
function renderLastBackupInfo() {
  ...
  if (last) el.textContent = 'Ø¢Ø®Ø± ÙØ³Ø®Ø© Ø§Ø­ØªÙØ§Ø·ÙØ©: ' + new Date(last).toLocaleString('ar-EG');
}

function printReport(sectionId) {
  const html = document.getElementById(sectionId).innerHTML;
  const w = window.open('','_blank');
  w.document.write('<html dir="rtl"><head><title>ØªÙØ±ÙØ±</title>'
...
function renderBranchUsersSettings() {
  ...
  container.innerHTML = BRANCH_IDS.map(b => {
    ...
    return `<div style="border:1px solid var(--border); border-radius:8px; padding:10px 12px; margin-bottom:10px; background:var(--bg);">
      <div style="font-weight:700; font-size:13px; margin-bottom:8px; color:var(--primary);">ð¬ ${name}</div>
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px;">
        <div><label style="font-size:11px; color:var(--text-muted);">Ø§Ø³Ù Ø§ÙÙØ³ØªØ®Ø¯Ù</label>
          <input class="form-control" id="bu-user-${b}" value="${uname}" placeholder="username" style="margin-top:4px;" /></div>
        <div><label style="font-size:11px; color:var(--text-muted);">ÙÙÙØ© Ø§ÙÙØ±ÙØ±</label>
...
async function saveBranchUsers() {
  ...
  showMsg('sBranchUsersMsg', 'â ØªÙ Ø­ÙØ¸ Ø¨ÙØ§ÙØ§Øª Ø¯Ø®ÙÙ Ø§ÙÙØ±ÙØ¹');
}
...
function changePass(role) {
  ...
  checkPass(curr, users.admin).then(ok => {
    if (!ok) { showMsg('sAdminMsg','ÙÙÙØ© Ø§ÙÙØ±ÙØ± Ø§ÙØ­Ø§ÙÙØ© ØºÙØ·','danger'); return; }
    if (np.length < 4) { showMsg('sAdminMsg','ÙØ¬Ø¨ Ø£Ù ØªÙÙÙ 4 Ø£Ø­Ø±Ù Ø¹ÙÙ Ø§ÙØ£ÙÙ','danger'); return; }
    hashPass(np).then(hashed => {
      ...
      showMsg('sAdminMsg','â ØªÙ ØªØºÙÙØ± ÙÙÙØ© Ø§ÙÙØ±ÙØ±');
...
function saveSettings() {
  ...
  saveSettingsCache();
  showMsg('sSettingsMsg','ØªÙ Ø­ÙØ¸ Ø§ÙØ¥Ø¹Ø¯Ø§Ø¯Ø§Øª');
}
...
function renderSellersSettings() {
  ...
  wrap.innerHTML = people.map((n,i) => `
    <div style="display:flex;align-items:center;gap:6px;background:#f8fafc;border:1px solid var(--border);border-radius:20px;padding:5px 12px;font-size:13px;font-weight:600;">
      <span>ð¤ ${n}</span>
      <button onclick="removeSeller(${i})" style="background:none;border:none;cursor:pointer;color:var(--danger);font-size:14px;line-height:1;padding:0 2px;" title="Ø­Ø°Ù">Ã</button>
    </div>`).join('');
}
...
function addSeller() {
  ...
  if (arr.includes(name)) { showMsg('sSellersMsg','Ø§ÙØ§Ø³Ù ÙÙØ¬ÙØ¯ Ø¨Ø§ÙÙØ¹Ù','danger'); return; }
  arr.push(name);
  setSalespeople(arr);
  inp.value = '';
  showMsg('sSellersMsg','ØªÙØª Ø§ÙØ¥Ø¶Ø§ÙØ©');
}

function removeSeller(idx) {
  const arr = [...getSalespeople()];
  if (arr.length <= 1) { showMsg('sSellersMsg','ÙØ§Ø²Ù ÙÙÙÙ ÙÙÙ Ø¨Ø§Ø¦Ø¹ ÙØ§Ø­Ø¯ Ø¹ÙÙ Ø§ÙØ£ÙÙ','danger'); return; }
  arr.splice(idx, 1);
  setSalespeople(arr);
  showMsg('sSellersMsg','ØªÙ Ø§ÙØ­Ø°Ù');
}

function resetSales() {
  if (!confirm('Ø­Ø°Ù ÙÙ Ø§ÙÙØ¨ÙØ¹Ø§Øª ÙÙØ§Ø¦ÙØ§ÙØ')) return;
  setSales([]);
  showMsg('sSettingsMsg','ØªÙ Ø­Ø°Ù Ø³Ø¬Ù Ø§ÙÙØ¨ÙØ¹Ø§Øª','warning');
}

function resetAll() {
  if (!confirm('Ø­Ø°Ù ÙÙ Ø§ÙØ¨ÙØ§ÙØ§Øª (Ø§ÙÙØ®Ø²ÙÙ + Ø§ÙÙØ¨ÙØ¹Ø§Øª)Ø')) return;
  if (!confirm('ØªØ£ÙÙØ¯ Ø£Ø®ÙØ± â ÙØ°Ø§ ÙØ§ ÙÙÙÙ Ø§ÙØªØ±Ø§Ø¬Ø¹ Ø¹ÙÙ')) return;
  ...
  showMsg('sSettingsMsg','ØªÙ Ø­Ø°Ù ÙÙ Ø§ÙØ¨ÙØ§ÙØ§Øª','danger');
}
...
// ââ PASSWORD SECURITY âââââââââââââââââââââââââââââââââââââââââ
...
// setUsers / setBranchUsers â NEVER sync passwords to Firestore
...
function initFirebase() {
  ...
  if (!FIREBASE_CONFIG.projectId) {
    ...
    if (fbEl) fbEl.innerHTML = 'Ø¨Ø¯ÙÙ Firebase';
    return;
  }
  ...
  try {
    ...
    if (fbEl) {
      ...
      fbEl.textContent   = 'Ø¬Ø§Ø±Ù Ø§ÙØ§ØªØµØ§Ù...';
    }
    // Anonymous auth â required for Firestore Security Rules
    ...
    _db.collection('pos_data').doc('suspended')
      .onSnapshot(snap => {
        ...
        if (fbEl) {
          ...
          fbEl.textContent   = 'Firebase ÙØªØµÙ';
        }
      }, err => {
        ...
        if (fbEl) {
          ...
          fbEl.textContent   = 'Ø®Ø·Ø£ ÙÙ Ø§ÙØ§ØªØµØ§Ù';
        }
      });

    initApprovalsFirebaseListener();
    // ââ branch inventory listeners (wh + b1..b4) ââââââââââââââââââââââââââââââââââ
    BRANCH_IDS.forEach(b => {
      _db.collection('pos_data').doc(`inv_${b}`)
        .onSnapshot(snap => {
          if (snap.exists) {
            ...
            // Migration: first load â try old single-branch 'inv' for b1, then localStorage
            const local = b === 'b1'
    ...
    _db.collection('pos_data').doc('settings')
      .onSnapshot(snap => {
        if (snap.exists) {
          ...
          if (!_settingsCache.salespeople) _settingsCache.salespeople = DB.g('salespeople', ['ÙØ­ÙØ¯','Ø§ÙØ§Ø¡']);
        } else {
          const localThresh      = DB.g('threshold', 5);
          const localSalespeople = DB.g('salespeople', ['ÙØ­ÙØ¯','Ø§ÙØ§Ø¡']);
    ...
    // Auth listener â sync passwords across devices
    ...
    _settingsCache  = { threshold: DB.g('threshold', 5) };
    if (fbEl) fbEl.textContent = 'Ø®Ø·Ø£ Firebase';
  }
}
...
function suspendBill() {
  ...
  document.getElementById('suspendCancelBtn').textContent = 'Ø¥ÙØºØ§Ø¡';
...
function confirmSuspend() {
  ...
  document.getElementById('suspendSuccessText').innerHTML =
    `ØªÙ ØªØ¹ÙÙÙ Ø§ÙÙØ§ØªÙØ±Ø© <strong style="font-family:monospace; color:var(--primary); font-size:16px;">${bill.id}</strong><br>
    <span style="font-size:12px; color:var(--text-muted); margin-top:4px; display:block;">Ø£Ø¨ÙØº Ø§ÙÙØ¯ÙØ± â ÙÙØ§ÙÙÙØ§ ÙÙ "ÙÙØ§ØªÙØ± ÙØ¹ÙÙØ©"</span>`;
  msg.classList.remove('hidden');
  document.getElementById('suspendConfirmBtn').classList.add('hidden');
  document.getElementById('suspendCancelBtn').textContent = 'Ø¥ØºÙØ§Ù';
}
...
function renderSuspendedPage() {
  const list = getSuspended();
  document.getElementById('suspendedCountLabel').textContent = list.length + ' ÙØ§ØªÙØ±Ø© ÙØ¹ÙÙØ©';
  const el = document.getElementById('suspendedList');
  if (!list.length) {
    el.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ ÙÙØ§ØªÙØ± ÙØ¹ÙÙØ©</div>';
    return;
  }
    ...
    return `<div class="card" style="margin-bottom:12px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px;margin-bottom:10px;">
        <div>
          ...
          <div style="font-size:12px;color:var(--text-muted);">${new Date(b.created).toLocaleString('ar-EG')} â ÙØ§Ø´ÙØ±: ${b.cashier}</div>
          ${b.note?`<div style="font-size:12px;margin-top:3px;color:#6b7280;">ÙÙØ§Ø­Ø¸Ø©: ${b.note}</div>`:''}
        </div>
        <div style="text-align:left;">
          <div style="font-size:18px;font-weight:700;">${fmt(final)} Ø¬</div>
          ${discAmt>0?`<div style="font-size:12px;color:var(--success);">Ø®ØµÙ: -${fmt(discAmt)} Ø¬</div>`:''}
          ${b.adminDiscountNote?`<div style="font-size:11px;color:var(--text-muted);">${b.adminDiscountNote}</div>`:''}
        </div>
      </div>
      <div style="font-size:12px;color:var(--text-muted);margin-bottom:10px;">
        ${b.items.map(i=>`${i.name} Ã ${i.qty}`).join(' Â· ')}
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <button class="btn btn-primary btn-sm" onclick="activateSuspended('${b.id}')">ØªÙØ¹ÙÙ ÙÙÙØ§Ø´ÙØ±</button>
        <button class="btn btn-warning btn-sm" onclick="openAdminDiscount('${b.id}')">Ø¥Ø¶Ø§ÙØ© Ø®ØµÙ</button>
        <button class="btn btn-danger btn-sm" onclick="deleteSuspended('${b.id}')">Ø­Ø°Ù</button>
      </div>
    </div>`;
  }).join('');
}
...
function activateSuspended(id) {
  ...
  // Prices are already modified by admin â no need for extra _adminDiscount
  ...
  renderCart(); renderProducts();
  document.getElementById('adminDiscountRow').classList.add('hidden');
  alert('ØªÙ ØªØ­ÙÙÙ Ø§ÙÙØ§ØªÙØ±Ø© â ÙÙÙÙ Ø§ÙØ¢Ù Ø§ÙØ¯ÙØ¹ ÙÙ Ø§ÙÙØ§Ø´ÙØ±');
}

function deleteSuspended(id) {
  if (!confirm('Ø­Ø°Ù ÙØ°Ù Ø§ÙÙØ§ØªÙØ±Ø© Ø§ÙÙØ¹ÙÙØ©Ø')) return;
  setSuspended(getSuspended().filter(b => b.id !== id));
  renderSuspendedPage();
}
...
function openAdminDiscount(id) {
  ...
  tbody.innerHTML = bill.items.map((item, idx) => {
    ...
    return `<tr data-orig-price="${origPrice}" data-qty="${item.qty}">
      ...
      <td style="text-align:center;">${fmt(origPrice)} Ø¬</td>
      ...
      <td id="ad-final-${idx}" style="text-align:center;font-weight:600;">0 Ø¬</td>
    </tr>`;
  }).join('');
  calcAdminDiscount();
...
function calcAdminDiscount() {
  ...
  bill.items.forEach((item, idx) => {
    ...
    if (el) {
      el.textContent = (diff >= 0 ? '+' : '') + fmt(diff) + ' Ø¬';
      el.style.color = diff > 0 ? 'var(--danger)' : diff < 0 ? 'var(--success)' : 'var(--text-muted)';
    }
  });
  document.getElementById('adSubTotal').textContent    = fmt(totalOrig) + ' Ø¬';
  document.getElementById('adDiscountCalc').textContent = fmt(totalNew) + ' Ø¬';
  const netDiff = totalNew - totalOrig;
  const finalEl = document.getElementById('adFinalTotal');
  finalEl.textContent = (netDiff >= 0 ? '+' : '') + fmt(netDiff) + ' Ø¬';
...
function saveAdminDiscount() {
  ...
  renderSuspendedPage();
  addAuditLog('discount.apply', `ØªØ¹Ø¯ÙÙ Ø£Ø³Ø¹Ø§Ø± ÙØ§ØªÙØ±Ø© ÙØ¹ÙÙØ© â ÙÙ ${fmt(totalOrig)} Ø¥ÙÙ ${fmt(totalNew)} Ø¬`, currentBranch);
}
...
function renderCashierReport() {
  ...
  // All sales for this branch in the date range (no cashier filter â show all)
  ...
  document.getElementById('crpKPIs').innerHTML = [
    {label:'Ø¥Ø¬ÙØ§ÙÙ Ø§ÙÙØ¨ÙØ¹Ø§Øª', val:fmt(totalRev)+' Ø¬',  bg:'#dcfce7',tc:'#15803d',icon:'ð°'},
    {label:'Ø¹Ø¯Ø¯ Ø§ÙÙÙØ§ØªÙØ±',    val:invoiceQty,            bg:'#eff6ff',tc:'#1d4ed8',icon:'ð§¾'},
    {label:'ÙØªÙØ³Ø· Ø§ÙÙØ§ØªÙØ±Ø©',  val:fmt(atv)+' Ø¬',         bg:'#fef9c3',tc:'#854d0e',icon:'ð'},
    {label:'Ø¥Ø¬ÙØ§ÙÙ Ø§ÙÙØ±ØªØ¬Ø¹Ø§Øª',val:fmt(returnAmt)+' Ø¬',   bg:'#fee2e2',tc:'#b91c1c',icon:'ð'},
  ...
  document.getElementById('crpTopProducts').innerHTML = topProds.length
    ? `<div style="background:var(--bg-secondary);border-radius:10px;padding:12px;margin-bottom:4px;">
        <div style="font-size:13px;font-weight:700;margin-bottom:8px;">ð Ø£ÙØ«Ø± Ø§ÙÙÙØªØ¬Ø§Øª ÙØ¨ÙØ¹Ø§Ù</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px;">
          ${topProds.map(([name,d],i)=>`<span style="background:white;border:1px solid var(--border);border-radius:20px;padding:4px 12px;font-size:12px;display:flex;align-items:center;gap:5px;">
            ...
            ${name} <span style="color:var(--primary);font-weight:700;">${d.qty} ÙØ·Ø¹Ø©</span></span>`).join('')}
        </div>
      </div>` : '';
  ...
  if(!days.length){
    body.innerHTML='<p style="text-align:center;padding:30px;color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ ÙØ¨ÙØ¹Ø§Øª ÙÙ ÙØ°Ù Ø§ÙÙØªØ±Ø©</p>';
    return;
  }
  body.innerHTML = days.map(day=>{
    ...
    const invoicesHtml = d.sales.sort((a,b)=>b.id-a.id).map(s=>{
      ...
      const preview = (s.items||[]).map(i=>i.name+'Ã'+i.qty).join('Ø ');
      const pmColor = s.payMethod==='card'?'#1d4ed8':s.payMethod==='mixed'?'#7c3aed':'#15803d';
      const pmLabel = s.payMethod==='card'?'ÙØ§Ø±Øª':s.payMethod==='mixed'?'ÙØ®ØªÙØ·':'ÙØ§Ø´';
      return `<div style="display:flex;justify-content:space-between;align-items:flex-start;padding:8px 10px;border-radius:6px;background:var(--bg-secondary);margin-bottom:4px;gap:8px;flex-wrap:wrap;">
        <div style="min-width:0;flex:1;">
          ...
          ${s.customerName?`<span style="font-size:11px;color:var(--text-muted);margin-inline-start:6px;">ð¤ ${s.customerName}</span>`:''}
          <div style="font-size:11px;color:var(--text-muted);margin-top:2px;">${preview}</div>
        </div>
        <div style="text-align:end;flex-shrink:0;">
          <div style="font-weight:700;font-size:13px;">${fmt(s.total)} Ø¬</div>
    ...
    return `<div style="border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:12px;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
        ...
        <div style="display:flex;gap:8px;align-items:center;">
          <span style="font-size:11px;color:var(--text-muted);">${d.count} ÙØ§ØªÙØ±Ø©</span>
          <span style="background:#dcfce7;color:#15803d;padding:4px 12px;border-radius:10px;font-weight:700;font-size:13px;">${fmt(d.total)} Ø¬</span>
        </div>
      </div>
...
function renderResumeList() {
  ...
  if (!list.length) {
    el.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ ÙÙØ§ØªÙØ± ÙØ¹ÙÙØ©</div>';
    return;
  }
  el.innerHTML = list.map(b => {
    const disc    = b.adminDiscount || 0;
    ...
    return `<div style="border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:10px;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
        ...
        <span style="font-size:15px;font-weight:700;">${fmt(final)} Ø¬</span>
      </div>
      <div style="font-size:11px;color:var(--text-muted);margin-bottom:8px;">${new Date(b.created).toLocaleString('ar-EG')}</div>
      <div style="font-size:12px;margin-bottom:8px;">${b.items.map(i=>i.name+' Ã'+i.qty).join(' Â· ')}</div>
      ${discAmt>0?`<div style="font-size:12px;color:var(--success);margin-bottom:8px;">Ø®ØµÙ ÙØ¯ÙØ±: -${fmt(discAmt)} Ø¬</div>`:''}
      ${b.note?`<div style="font-size:12px;color:#6b7280;margin-bottom:8px;">ÙÙØ§Ø­Ø¸Ø©: ${b.note}</div>`:''}
      <button class="btn btn-success btn-sm" onclick="resumeFromModal('${b.id}')">Ø§Ø³ØªØ¦ÙØ§Ù ÙØ°Ù Ø§ÙÙØ§ØªÙØ±Ø©</button>
    </div>`;
  }).join('');
}
...
function saveBranchNames() {
  ...
  loadBranchNamesUI();
  showMsg('sBranchMsg', 'ØªÙ Ø­ÙØ¸ Ø£Ø³ÙØ§Ø¡ Ø§ÙÙØ±ÙØ¹ â', 'success');
}

// ââ TRANSFERS ââââââââââââââââââââââââââââââââââ
...
function addTrProductSearch() {
  ...
  if (!matches.length) { dd.innerHTML='<div style="padding:8px;font-size:12px;color:var(--text-muted);">ÙØ§ ÙØªØ§Ø¦Ø¬</div>'; dd.classList.remove('hidden'); return; }
  dd.innerHTML = matches.slice(0,8).map(p=>`
    <div style="padding:7px 10px;cursor:pointer;font-size:13px;border-bottom:1px solid #f0f0f0;"
      ...
      <span>${p.name}</span>
      <span style="color:var(--text-muted);font-size:11px;margin-right:6px;">ÙØªØ§Ø­: ${p.qty}</span>
    </div>`).join('');
...
function renderTrItems() {
  ...
  if (!_trItems.length) { el.innerHTML='<div style="color:var(--text-muted);font-size:12px;padding:8px;">ÙÙ ÙØªÙ Ø¥Ø¶Ø§ÙØ© Ø£ØµÙØ§Ù</div>'; return; }
  el.innerHTML = _trItems.map((item,idx) => `
    <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--border);">
      <span style="flex:1;font-size:13px;">${item.name}</span>
      <span style="font-size:11px;color:var(--text-muted);">ÙØªØ§Ø­: ${item.maxQty}</span>
      ...
      <button onclick="_trItems.splice(${idx},1);renderTrItems();" style="background:none;border:none;color:var(--danger);cursor:pointer;font-size:15px;padding:0 2px;">â</button>
    </div>`).join('');
}
...
function confirmTransfer() {
  ...
  if (from === to) { alert('Ø§ÙÙØ±Ø¹ Ø§ÙÙØµØ¯Ø± ÙØ§ÙÙØ¬ÙØ© ÙØªØ·Ø§Ø¨ÙØ§Ù'); return; }
  if (!_trItems.length) { alert('Ø£Ø¶Ù Ø£ØµÙØ§Ù ÙÙØªØ­ÙÙÙ'); return; }
  ...
  for (const item of _trItems) {
    ...
    if (!p || p.qty < item.qty) { alert(`Ø§ÙÙÙÙØ© Ø§ÙÙØ·ÙÙØ¨Ø© ÙÙ "${item.name}" ØºÙØ± ÙØªØ§Ø­Ø© ÙÙ Ø§ÙÙØµØ¯Ø±`); return; }
  }
  ...
  const record = {
    ...
    note,
    status: 'completed',
    by: currentUser === 'admin' ? 'ÙØ¯ÙØ±' : 'ÙØ§Ø´ÙØ±'
  };
  ...
  renderTransfersPage();
  alert(`â ØªÙ Ø§ÙØªØ­ÙÙÙ Ø¨ÙØ¬Ø§Ø­\nÙÙ: ${record.fromName} â Ø¥ÙÙ: ${record.toName}\n${_trItems.length} ØµÙÙ`);
}
...
function renderTransfersPage() {
  ...
  if (!list.length) {
    tbody.innerHTML = '<tr><td colspan="7" class="text-center text-muted" style="padding:24px;">ÙØ§ ØªÙØ¬Ø¯ ØªØ­ÙÙÙØ§Øª Ø¨Ø¹Ø¯</td></tr>';
    return;
  }
  tbody.innerHTML = list.map(t => `<tr>
    ...
    <td style="font-size:12px;">${t.items.map(i=>`${i.name} Ã${i.qty}`).join(' Â· ')}</td>
    <td><span style="background:${t.status==='completed'?'#d1fae5':'#fef9c3'};color:${t.status==='completed'?'#065f46':'#92400e'};padding:2px 8px;border-radius:10px;font-size:11px;">${t.status==='completed'?'â ÙÙØªÙÙ':'â³ ÙØ¹ÙÙ'}</span></td>
...
function openSupplierModal(id) {
  ...
  document.getElementById('supModalTitle').textContent = sup ? 'âï¸ ØªØ¹Ø¯ÙÙ ÙÙØ±Ø¯' : 'â ÙÙØ±Ø¯ Ø¬Ø¯ÙØ¯';
...
function saveSupplier() {
  ...
  if (!name) { alert('Ø£Ø¯Ø®Ù Ø§Ø³Ù Ø§ÙÙÙØ±Ø¯'); return; }
...
function deleteSupplier(id) {
  if (!confirm('Ø­Ø°Ù ÙØ°Ø§ Ø§ÙÙÙØ±Ø¯ ÙÙØ§Ø¦ÙØ§ÙØ')) return;
...
function renderSuppliersPage() {
  ...
  const elD = document.getElementById('sup-debt'); if (elD) elD.textContent = fmt(totalDebt) + ' Ø¬';
  const tbody = document.getElementById('suppliersBody'); if (!tbody) return;
  if (!list.length) {
    tbody.innerHTML = '<tr><td colspan="6" class="text-center text-muted" style="padding:24px;">ÙØ§ ÙÙØ¬Ø¯ ÙÙØ±Ø¯ÙÙ Ø¨Ø¹Ø¯ â Ø£Ø¶Ù Ø£ÙÙ ÙÙØ±Ø¯</td></tr>';
    return;
  }
  tbody.innerHTML = list.map(s => `<tr>
    ...
    <td style="font-weight:600; color:${(s.balance||0)>0?'var(--danger)':'var(--success)'};">${fmt(s.balance||0)} Ø¬</td>
    <td style="font-size:12px; color:var(--text-muted);">${s.notes || '-'}</td>
    <td>
      <button class="btn btn-sm" onclick="openSupplierModal('${s.id}')" style="font-size:11px; padding:3px 8px; margin-left:4px;">âï¸</button>
      <button class="btn btn-danger btn-sm" onclick="deleteSupplier('${s.id}')" style="font-size:11px; padding:3px 8px;">ðï¸</button>
    </td>
  </tr>`).join('');
}
...
function openPOModal(id) {
  ...
  document.getElementById('poModalTitle').textContent = po ? 'âï¸ ØªØ¹Ø¯ÙÙ Ø£ÙØ± Ø§ÙØ´Ø±Ø§Ø¡' : 'ð Ø£ÙØ± Ø´Ø±Ø§Ø¡ Ø¬Ø¯ÙØ¯';
  document.getElementById('poEditId').value = po?.id || '';
  // Populate supplier dropdown
  const supSel = document.getElementById('poSupplierId');
  supSel.innerHTML = '<option value="">-- Ø§Ø®ØªØ± ÙÙØ±Ø¯ --</option>' +
...
function poProdSearchFn() {
  ...
  dd.innerHTML = results.map(i => `<div style="padding:8px 12px; cursor:pointer; font-size:13px; border-bottom:1px solid var(--border);"
    ...
    <span style="color:var(--primary); font-size:11px;">ØªÙÙÙØ©: ${fmt(i.cost||0)} Ø¬</span>
  </div>`).join('');
  dd.classList.remove('hidden');
}
...
function updatePOItem(code, field, val) {
  ...
  if (totalEl) totalEl.textContent = fmt(item.qty * item.cost) + ' Ø¬';
}
...
function renderPOItems() {
  ...
  if (!_poItems.length) {
    tbody.innerHTML = '<tr><td colspan="5" class="text-center text-muted" style="padding:16px;">ÙÙ ØªÙØ¶Ù Ø£ØµÙØ§Ù Ø¨Ø¹Ø¯</td></tr>';
    return;
  }
  tbody.innerHTML = _poItems.map(i => `<tr>
    ...
    <td id="po-line-total-${i.code}" style="font-weight:600;">${fmt(i.qty*i.cost)} Ø¬</td>
    <td><button class="btn btn-danger btn-sm" onclick="removePOItem('${i.code}')" style="padding:2px 6px; font-size:11px;">â</button></td>
  </tr>`).join('');
}
...
function calcPOTotals() {
  ...
  const s = document.getElementById('poSubtotal'); if (s) s.textContent = fmt(subtotal) + ' Ø¬';
  const sd = document.getElementById('poShippingDisp'); if (sd) sd.textContent = fmt(shipping) + ' Ø¬';
  const g = document.getElementById('poGrandTotal'); if (g) g.textContent = fmt(grand) + ' Ø¬';
}

function savePO() {
  const supplierId = document.getElementById('poSupplierId').value;
  if (!supplierId) { alert('Ø§Ø®ØªØ± Ø§ÙÙÙØ±Ø¯ Ø£ÙÙØ§Ù'); return; }
  if (!_poItems.length) { alert('Ø£Ø¶Ù ØµÙÙ ÙØ§Ø­Ø¯ Ø¹ÙÙ Ø§ÙØ£ÙÙ'); return; }
...
function openPODetails(id) {
  ...
  if (!po) return;
  _poViewId = id;
  document.getElementById('poDetailsTitle').textContent = `ð Ø£ÙØ± Ø§ÙØ´Ø±Ø§Ø¡ #${po.id.slice(-6)}`;
  const statusLabel = po.status === 'received' ? 'â ÙØ³ØªÙÙØ©' : po.status === 'partial' ? 'ð¶ Ø¬Ø²Ø¦Ù' : 'â³ ÙÙØ¯ Ø§ÙØ§ÙØªØ¸Ø§Ø±';
  ...
  document.getElementById('poDetailsBody').innerHTML = `
    <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:12px; margin-bottom:16px; background:var(--bg); border-radius:8px; padding:12px; border:1px solid var(--border);">
      <div><div style="font-size:11px;color:var(--text-muted);">Ø§ÙÙÙØ±Ø¯</div><div style="font-weight:700;">${po.supplierName}</div></div>
      <div><div style="font-size:11px;color:var(--text-muted);">Ø§ÙÙØ±Ø¹</div><div style="font-weight:700;">${getBranchName(po.branchId)}</div></div>
      <div><div style="font-size:11px;color:var(--text-muted);">Ø§ÙØ­Ø§ÙØ©</div><span style="background:${statusBg};color:${statusColor};padding:2px 10px;border-radius:10px;font-size:12px;">${statusLabel}</span></div>
      <div><div style="font-size:11px;color:var(--text-muted);">ØªØ§Ø±ÙØ® Ø§ÙØ¥ÙØ´Ø§Ø¡</div><div style="font-size:12px;">${new Date(po.createdAt).toLocaleDateString('ar-EG')}</div></div>
      ${po.expectedDate?`<div><div style="font-size:11px;color:var(--text-muted);">ÙÙØ¹Ø¯ Ø§ÙØ§Ø³ØªÙØ§Ù</div><div style="font-size:12px;">${po.expectedDate}</div></div>`:''}
      ${po.receivedAt?`<div><div style="font-size:11px;color:var(--text-muted);">ØªØ§Ø±ÙØ® Ø§ÙØ§Ø³ØªÙØ§Ù</div><div style="font-size:12px;color:var(--success);">${new Date(po.receivedAt).toLocaleDateString('ar-EG')}</div></div>`:''}
    </div>
    <table style="width:100%; border-collapse:collapse; margin-bottom:12px;">
      <thead><tr style="background:var(--bg);">
        <th style="padding:8px; text-align:right; border:1px solid var(--border);">Ø§ÙØµÙÙ</th>
        <th style="padding:8px; text-align:center; border:1px solid var(--border);">Ø§ÙÙÙÙØ©</th>
        <th style="padding:8px; text-align:center; border:1px solid var(--border);">Ø§ÙØªÙÙÙØ©</th>
        <th style="padding:8px; text-align:center; border:1px solid var(--border);">Ø§ÙØ¥Ø¬ÙØ§ÙÙ</th>
      </tr></thead>
      <tbody>${po.items.map(i=>`<tr>
        ...
        <td style="padding:8px; text-align:center; border:1px solid var(--border);">${fmt(i.cost)} Ø¬</td>
        <td style="padding:8px; text-align:center; border:1px solid var(--border);">${fmt(i.qty*i.cost)} Ø¬</td>
      </tr>`).join('')}</tbody>
    </table>
    <div style="text-align:left; font-size:13px;">
      <div>Ø¥Ø¬ÙØ§ÙÙ Ø§ÙØ£ØµÙØ§Ù: <strong>${fmt(po.subtotal)} Ø¬</strong></div>
      <div>Ø§ÙØ´Ø­Ù: <strong>${fmt(po.shipping||0)} Ø¬</strong></div>
      <div style="font-size:16px; margin-top:4px;">Ø§ÙØ¥Ø¬ÙØ§ÙÙ Ø§ÙÙÙÙ: <strong style="color:var(--primary);">${fmt(po.total)} Ø¬</strong></div>
    </div>
    ${po.notes?`<div style="margin-top:12px; padding:8px; background:#fffbeb; border-radius:6px; font-size:13px;">ð ${po.notes}</div>`:''}
  `;
...
function receivePO() {
  ...
  if (!po) return;
  if (!confirm(`Ø§Ø³ØªÙØ§Ù Ø¨Ø¶Ø§Ø¹Ø© Ø£ÙØ± Ø§ÙØ´Ø±Ø§Ø¡ #${po.id.slice(-6)}Ø\nØ³ÙØªÙ ØªØ­Ø¯ÙØ« ÙØ®Ø²ÙÙ ÙØ±Ø¹: ${getBranchName(po.branchId)}\nÙØªØ­Ø¯ÙØ« ØªÙÙÙØ© Ø§ÙØ£ØµÙØ§Ù Ø¨ÙØ§ ÙÙ Ø°ÙÙ ØªÙÙÙØ© Ø§ÙØ´Ø­Ù Ø§ÙÙÙØ²Ø¹Ø©.`)) return;
  ...
  addAuditLog('po.receive', `Ø§Ø³ØªÙØ§Ù Ø£ÙØ± Ø´Ø±Ø§Ø¡ #${po.id.slice(-6)} â ${getBranchName(po.branchId)} â ${fmt(po.total)} Ø¬`, po.branchId);
  closeModal('poDetailsModal');
  renderPurchasesPage();
  alert(`â ØªÙ Ø§ÙØ§Ø³ØªÙØ§Ù Ø¨ÙØ¬Ø§Ø­!\nØªÙ ØªØ­Ø¯ÙØ« ÙØ®Ø²ÙÙ ${getBranchName(po.branchId)} ÙØªÙØ§ÙÙÙ Ø§ÙØ£ØµÙØ§Ù.`);
}

function deletePO(id) {
  if (!confirm('Ø­Ø°Ù Ø£ÙØ± Ø§ÙØ´Ø±Ø§Ø¡ ÙØ°Ø§Ø')) return;
...
function populatePOFilters() {
  ...
  if (supSel) {
    supSel.innerHTML = '<option value="">Ø¬ÙÙØ¹ Ø§ÙÙÙØ±Ø¯ÙÙ</option>' +
      getSuppliers().map(s => `<option value="${s.id}">${s.name}</option>`).join('');
  }
  const brSel = document.getElementById('poBranchFilter');
  if (brSel) {
    brSel.innerHTML = '<option value="">Ø¬ÙÙØ¹ Ø§ÙÙØ±ÙØ¹</option>' +
...
function renderPurchasesPage() {
  ...
  const elT = document.getElementById('po-total');    if (elT) elT.textContent = fmt(total) + ' Ø¬';
  ...
  if (!list.length) {
    tbody.innerHTML = '<tr><td colspan="9" class="text-center text-muted" style="padding:24px;">ÙØ§ ØªÙØ¬Ø¯ Ø£ÙØ§ÙØ± Ø´Ø±Ø§Ø¡ Ø¨Ø¹Ø¯</td></tr>';
    return;
  }
  tbody.innerHTML = list.slice().reverse().map(po => {
    const statusLabel = po.status==='received'?'â ÙØ³ØªÙÙØ©':po.status==='partial'?'ð¶ Ø¬Ø²Ø¦Ù':'â³ Ø§ÙØªØ¸Ø§Ø±';
    const statusBg    = po.status==='received'?'#d1fae5':po.status==='partial'?'#fef9c3':'#dbeafe';
    const statusColor = po.status==='received'?'#065f46':po.status==='partial'?'#92400e':'#1d4ed8';
    return `<tr>
      ...
      <td style="font-size:12px;">${po.items.length} ØµÙÙ</td>
      <td>${fmt(po.shipping||0)} Ø¬</td>
      <td style="font-weight:700; color:var(--primary);">${fmt(po.total)} Ø¬</td>
      ...
      <td>
        <button class="btn btn-sm" onclick="openPODetails('${po.id}')" style="font-size:11px; padding:3px 8px; margin-left:4px;">ðï¸</button>
        ${po.status!=='received'?`<button class="btn btn-sm" onclick="openPOModal('${po.id}')" style="font-size:11px; padding:3px 8px; margin-left:4px;">âï¸</button>`:''}
        <button class="btn btn-danger btn-sm" onclick="deletePO('${po.id}')" style="font-size:11px; padding:3px 8px;">ðï¸</button>
      </td>
    </tr>`;
  }).join('');
}
...
// ââââââââââââââââââââââââââââââââââââââââââââââ
// HR SYSTEM â TARGETS & COMMISSION
// ââââââââââââââââââââââââââââââââââââââââââââââ
...
function openHRTargetModal() {
  ...
  if (!salespeople.length) {
    tbody.innerHTML = '<tr><td colspan="3" class="text-center text-muted" style="padding:16px;">ÙØ§ ÙÙØ¬Ø¯ Ø¨Ø§Ø¦Ø¹ÙÙ â Ø£Ø¶ÙÙÙ ÙÙ Ø§ÙØ¥Ø¹Ø¯Ø§Ø¯Ø§Øª Ø£ÙÙØ§Ù</td></tr>';
  } else {
...
function saveHRTargets() {
  ...
  if (!month) { alert('Ø§Ø®ØªØ± Ø§ÙØ´ÙØ±'); return; }
  const salespeople = (getSalespeople ? getSalespeople() : (DB.g('pos_salespeople',[])))
    .map(sp => typeof sp === 'string' ? sp : sp.name);
  const targets = {};
...
function populateHRMonthFilter() {
  ...
  sel.innerHTML = `<option value="">Ø§ÙØ´ÙØ± Ø§ÙØ­Ø§ÙÙ (${today})</option>` +
...
function renderHRPage() {
  ...
  const salesBySP = {};
  allSales.forEach(s => {
    const sp = s.salesperson || 'ØºÙØ± ÙØ­Ø¯Ø¯';
  ...
  const el1 = document.getElementById('hr-total-target');    if (el1) el1.textContent = fmt(totalTarget) + ' Ø¬';
  const el2 = document.getElementById('hr-total-sales');     if (el2) el2.textContent = fmt(totalSales) + ' Ø¬';
  const el3 = document.getElementById('hr-achieve-pct');     if (el3) el3.textContent = achievePct + '%';
  const el4 = document.getElementById('hr-total-commission'); if (el4) el4.textContent = fmt(totalComm) + ' Ø¬';

  // Render cards
  const container = document.getElementById('hrSalespeople'); if (!container) return;
  if (!rows.length) {
    container.innerHTML = '<div class="card" style="text-align:center; padding:32px; color:var(--text-muted);">ÙØ§ ÙÙØ¬Ø¯ Ø¨Ø§Ø¦Ø¹ÙÙ â Ø£Ø¶ÙÙÙ ÙÙ Ø§ÙØ¥Ø¹Ø¯Ø§Ø¯Ø§Øª</div>';
    return;
  }
  container.innerHTML = rows.map(r => {
    const barColor = r.pct >= 100 ? 'var(--success)' : r.pct >= 70 ? 'var(--warning)' : 'var(--danger)';
    const badge    = r.pct >= 100 ? 'ð ØªØ¬Ø§ÙØ² Ø§ÙÙØ¯Ù!' : r.pct >= 70 ? 'ð Ø¬ÙØ¯' : 'â ï¸ Ø¯ÙÙ Ø§ÙÙØ¯Ù';
    return `<div class="card" style="margin-bottom:12px;">
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px; margin-bottom:12px;">
        <div style="font-size:17px; font-weight:700;">ð¤ ${r.name}</div>
      ...
      <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(130px,1fr)); gap:12px; margin-bottom:14px;">
        <div style="text-align:center; background:var(--bg); border-radius:8px; padding:10px;">
          <div style="font-size:11px; color:var(--text-muted); margin-bottom:4px;">ð¯ Ø§ÙÙØ¯Ù</div>
          <div style="font-size:18px; font-weight:700;">${fmt(r.target)} Ø¬</div>
        </div>
        <div style="text-align:center; background:var(--bg); border-radius:8px; padding:10px;">
          <div style="font-size:11px; color:var(--text-muted); margin-bottom:4px;">ð° Ø§ÙÙØ¨ÙØ¹Ø§Øª Ø§ÙÙØ¹ÙÙØ©</div>
          <div style="font-size:18px; font-weight:700; color:var(--success);">${fmt(r.actual)} Ø¬</div>
        </div>
        <div style="text-align:center; background:var(--bg); border-radius:8px; padding:10px;">
          <div style="font-size:11px; color:var(--text-muted); margin-bottom:4px;">ð ÙØ³Ø¨Ø© Ø§ÙØ¥ÙØ¬Ø§Ø²</div>
          <div style="font-size:18px; font-weight:700; color:${barColor};">${Math.round(r.pct)}%</div>
        </div>
        <div style="text-align:center; background:var(--bg); border-radius:8px; padding:10px;">
          <div style="font-size:11px; color:var(--text-muted); margin-bottom:4px;">ð¸ Ø§ÙØ¹ÙÙÙØ© (${r.commPct}%)</div>
          <div style="font-size:18px; font-weight:700; color:#7c3aed;">${fmt(r.commission)} Ø¬</div>
        </div>
      </div>
      <!-- Progress bar -->
      ...
      <div style="display:flex; justify-content:space-between; font-size:11px; color:var(--text-muted); margin-top:4px;">
        <span>0</span><span>${fmt(r.target)} Ø¬</span>
      </div>
    </div>`;
  }).join('');
}
...
function openPromoModal(id) {
  ...
  document.getElementById('promoModalTitle').textContent = promo ? 'ØªØ¹Ø¯ÙÙ Ø§ÙØ¹Ø±Ø¶' : 'Ø¥Ø¶Ø§ÙØ© Ø¹Ø±Ø¶ Ø¬Ø¯ÙØ¯';
...
function renderPromoBundleItems() {
  ...
  if (!_promoBundleItems.length) {
    el.innerHTML = '<div style="color:var(--text-muted); font-size:12px; padding:8px;">ÙÙ ÙØªÙ Ø¥Ø¶Ø§ÙØ© ÙÙØªØ¬Ø§Øª Ø¨Ø¹Ø¯</div>';
    return;
  }
  el.innerHTML = _promoBundleItems.map((bi, idx) => `
    <div style="display:flex; align-items:center; gap:8px; padding:6px 0; border-bottom:1px solid var(--border);">
      ...
      <span style="font-size:12px;">ÙÙÙØ©:</span>
      ...
      <button onclick="_promoBundleItems.splice(${idx},1); renderPromoBundleItems();"
        style="background:none; border:none; color:var(--danger); cursor:pointer; font-size:15px; padding:0 2px;">â</button>
    </div>`).join('');
}
...
function savePromo() {
  ...
  const type = document.getElementById('promoType').value;
  if (!name) { alert('Ø£Ø¯Ø®Ù Ø§Ø³Ù Ø§ÙØ¹Ø±Ø¶'); return; }
  ...
  if (type === 'bundle') {
    if (_promoBundleItems.length < 2) { alert('Ø£Ø¶Ù ÙÙØªØ¬ÙÙ Ø¹ÙÙ Ø§ÙØ£ÙÙ ÙÙØ­Ø²ÙØ©'); return; }
    const bPrice = parseFloat(document.getElementById('promoBundlePrice').value);
    if (!bPrice || bPrice <= 0) { alert('Ø£Ø¯Ø®Ù Ø³Ø¹Ø± Ø§ÙØ­Ø²ÙØ©'); return; }
    ...
    if (!minAmt || minAmt <= 0) { alert('Ø£Ø¯Ø®Ù Ø§ÙØ­Ø¯ Ø§ÙØ£Ø¯ÙÙ ÙÙÙØ§ØªÙØ±Ø©'); return; }
    if (!dVal  || dVal <= 0)   { alert('Ø£Ø¯Ø®Ù ÙÙÙØ© Ø§ÙØ®ØµÙ'); return; }
    ...
    if (!catName) { alert('Ø£Ø¯Ø®Ù Ø§Ø³Ù Ø§ÙÙØ¦Ø©'); return; }
    if (!catDisc || catDisc <= 0 || catDisc > 100) { alert('Ø£Ø¯Ø®Ù ÙØ³Ø¨Ø© Ø§ÙØ®ØµÙ (1-100)'); return; }
    promo.categoryName = catName;
    promo.categoryDisc = catDisc;
  } else if (type === 'bxgy') {
    if (!_promoBundleItems.length) { alert('Ø£Ø¶Ù Ø§ÙÙÙØªØ¬ Ø§ÙÙØ·ÙÙØ¨ Ø´Ø±Ø§Ø¤Ù'); return; }
...
function deletePromo(id) {
  if (!confirm('Ø­Ø°Ù ÙØ°Ø§ Ø§ÙØ¹Ø±Ø¶Ø')) return;
...
function renderPromosPage() {
  ...
  const promoCard = (p) => {
    ...
    if (p.type === 'bundle') {
      ...
      detail = `<div style="font-size:12px; color:var(--text-muted); margin-top:4px;">
        ${(p.items||[]).map(bi => `${bi.name} Ã${bi.minQty}`).join(' + ')}
      </div>
      <div style="font-size:13px; margin-top:6px;">
        Ø³Ø¹Ø± Ø§ÙØ­Ø²ÙØ©: <strong>${fmt(p.bundlePrice)} Ø¬</strong>
        ${saving > 0 ? `<span style="color:var(--success); margin-right:8px;">ÙÙØ± ${fmt(saving)} Ø¬</span>` : ''}
      </div>`;
    } else if (p.type === 'threshold') {
      const discLabel = p.discountType === 'percent' ? `${p.discountValue}%` : `${fmt(p.discountValue)} Ø¬`;
      detail = `<div style="font-size:13px; margin-top:6px;">Ø¹ÙØ¯ ÙØ§ØªÙØ±Ø© â¥ <strong>${fmt(p.minAmount)} Ø¬</strong> â Ø®ØµÙ <strong>${discLabel}</strong></div>`;
    } else if (p.type === 'category') {
      detail = `<div style="font-size:13px; margin-top:6px;">Ø®ØµÙ <strong>${p.categoryDisc}%</strong> Ø¹ÙÙ ÙØ¦Ø©: <strong>${p.categoryName}</strong></div>`;
    } else if (p.type === 'bxgy') {
      detail = `<div style="font-size:13px; margin-top:6px;">Ø§Ø´ØªØ±Ù <strong>${p.buyQty}</strong> ÙÙ ${(p.items||[]).map(i=>i.name).join('+')} â Ø®Ø¯ <strong>${p.getQty}</strong> ÙØ¬Ø§ÙØ§Ù</div>`;
    }
    const dateRange = (p.startDate || p.endDate) ? `<div style="font-size:11px;color:var(--text-muted);margin-top:3px;">ð ${p.startDate||'â'} â ${p.endDate||'â'}</div>` : '';
      ...
      <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:8px;">
        <div style="flex:1;">
          <div style="font-weight:700; font-size:15px;">${p.name}
            ${p.active ? '<span style="background:#d1fae5; color:#065f46; font-size:11px; padding:2px 7px; border-radius:99px; margin-right:6px;">ÙØ´Ø·</span>'
                       : '<span style="background:#f3f4f6; color:#6b7280; font-size:11px; padding:2px 7px; border-radius:99px; margin-right:6px;">ÙØªÙÙÙ</span>'}
          </div>
          ${detail}${dateRange}
        </div>
        ${canEdit ? `<div style="display:flex; gap:6px; flex-shrink:0;">
          <button class="btn btn-sm ${p.active ? 'btn-gray' : 'btn-success'}" onclick="togglePromoActive(${p.id})">${p.active ? 'Ø¥ÙÙØ§Ù' : 'ØªÙØ¹ÙÙ'}</button>
          <button class="btn btn-sm btn-gray" onclick="openPromoModal(${p.id})">âï¸</button>
          <button class="btn btn-sm btn-danger" onclick="deletePromo(${p.id})">ðï¸</button>
        </div>` : ''}
      </div>
    </div>`;
  };
  ...
  if (bEl) bEl.innerHTML = bundles.length ? bundles.map(promoCard).join('') : '<div style="text-align:center; padding:24px; color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ Ø­Ø²Ù Ø¨Ø¹Ø¯</div>';
  if (tEl) tEl.innerHTML = thresholds.length ? thresholds.map(promoCard).join('') : '<div style="text-align:center; padding:24px; color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ Ø®ØµÙÙØ§Øª Ø¨Ø¹Ø¯</div>';
  const cEl = document.getElementById('categoriesBody');
  const xEl = document.getElementById('bxgyBody');
  if (cEl) cEl.innerHTML = categories.length ? categories.map(promoCard).join('') : '<div style="text-align:center; padding:24px; color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ Ø®ØµÙÙØ§Øª ÙØ¦Ø§Øª Ø¨Ø¹Ø¯</div>';
  if (xEl) xEl.innerHTML = bxgys.length ? bxgys.map(promoCard).join('') : '<div style="text-align:center; padding:24px; color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ Ø¹Ø±ÙØ¶ Ø§Ø´ØªØ±Ù ÙØ®Ø¯ Ø¨Ø¹Ø¯</div>';
...
function applyPromo(id) {
  ...
  if (discAmt <= 0) { alert('ÙØ°Ø§ Ø§ÙØ¹Ø±Ø¶ ÙØ§ ÙÙÙØ± Ø®ØµÙØ§Ù Ø¹ÙÙ Ø§ÙÙØ§ØªÙØ±Ø© Ø§ÙØ­Ø§ÙÙØ©'); return; }
...
function renderEligiblePromos() {
  ...
  el.innerHTML = eligible.map(promo => {
    ...
    const badge = promo.type === 'bundle' ? 'ð¦' : 'ð°';
    return `<div style="background:#faf5ff; border:1px dashed #7c3aed; border-radius:8px; padding:8px 10px; margin-bottom:6px; font-size:12px;">
      <div style="display:flex; justify-content:space-between; align-items:center; gap:6px;">
        <span>${badge} <strong>${promo.name}</strong> â ÙÙØ± ${fmt(disc)} Ø¬</span>
        <button class="btn btn-sm btn-primary" style="padding:3px 10px; font-size:11px;" onclick="applyPromo(${promo.id})">ØªØ·Ø¨ÙÙ</button>
      </div>
    </div>`;
  }).join('');
}
...
function renderCustomers() {
  ...
  const spentEl = document.getElementById('cust-spent'); if (spentEl) spentEl.textContent = fmt(totalSpent)+' Ø¬';
  const ptsEl   = document.getElementById('cust-pts');   if (ptsEl)   ptsEl.textContent   = totalPts+' ÙÙØ·Ø©';

  const tbody = document.getElementById('customersBody'); if (!tbody) return;
  if (!items.length) {
    tbody.innerHTML = '<tr><td colspan="7" class="text-center text-muted" style="padding:40px;">ÙØ§ ÙÙØ¬Ø¯ Ø¹ÙÙØ§Ø¡ â Ø£Ø¶Ù Ø¹ÙÙÙØ§Ù Ø¬Ø¯ÙØ¯Ø§Ù</td></tr>';
    return;
  }
  tbody.innerHTML = items.map(c => {
    ...
    if (spent >= 20000)      { tier='ð Ø¨ÙØ§ØªÙÙÙ'; tierColor='#4c1d95'; tierBg='#ede9fe'; }
    else if (spent >= 5000)  { tier='ð¥ Ø°ÙØ¨Ù';   tierColor='#78350f'; tierBg='#fef3c7'; }
    else if (spent >= 1000)  { tier='ð¥ ÙØ¶Ù';    tierColor='#1e3a5f'; tierBg='#dbeafe'; }
    else                     { tier='ð¥ Ø¨Ø±ÙÙØ²Ù';  tierColor='#4b3320'; tierBg='#fde8d0'; }
    return `<tr>
      <td><strong style="cursor:pointer;color:var(--primary);" onclick="openCustomerProfile('${c.id}')">${c.name}</strong></td>
      <td>${c.phone||'-'}</td>
      <td>${c.visits||0}</td>
      <td>${fmt(spent)} Ø¬</td>
      <td><span style="font-weight:700;color:#059669;">${pts} ÙÙØ·Ø©</span></td>
      ...
      <td>
        <button class="btn btn-sm" style="background:#e0f2fe;color:#0369a1;" onclick="openCustomerProfile('${c.id}')" title="Ø§ÙÙÙÙ Ø§ÙÙØ§ÙÙ">ðï¸</button>
        <button class="btn btn-gray btn-sm" onclick="openCustomerModal('${c.id}')" style="margin:0 3px;">âï¸</button>
        <button class="btn btn-danger btn-sm" onclick="deleteCustomer('${c.id}')">ðï¸</button>
      </td>
    </tr>`;
  }).join('');
}
...
function openCustomerModal(id) {
  ...
  document.getElementById('custModalTitle').textContent = c ? 'ØªØ¹Ø¯ÙÙ Ø¹ÙÙÙ' : 'Ø¥Ø¶Ø§ÙØ© Ø¹ÙÙÙ Ø¬Ø¯ÙØ¯';
...
function saveCustomer() {
  ...
  if (!name) { alert('Ø§ÙØ§Ø³Ù ÙØ·ÙÙØ¨'); return; }
...
function deleteCustomer(id) {
  if (!confirm('Ø­Ø°Ù ÙØ°Ø§ Ø§ÙØ¹ÙÙÙØ')) return;
  ...
  document.getElementById('cpBody').innerHTML = `
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px;">
      ...
      <div>
        ...
        <div style="font-size:13px;color:var(--text-muted);">${c.phone||'ÙØ§ ÙÙØ¬Ø¯ ÙØ§ØªÙ'}</div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:16px;">
      ${[
        {label:'Ø¥Ø¬ÙØ§ÙÙ Ø§ÙØ¥ÙÙØ§Ù', val:fmt(spent)+' Ø¬', bg:'#dcfce7', tc:'#15803d'},
        {label:'Ø¹Ø¯Ø¯ Ø§ÙØ²ÙØ§Ø±Ø§Øª',   val:visits,           bg:'#eff6ff', tc:'#1d4ed8'},
        {label:'ÙÙØ§Ø· Ø§ÙÙÙØ§Ø¡',    val:pts+' ÙÙØ·Ø©',      bg:'#fef9c3', tc:'#854d0e'},
        {label:'ÙÙÙØ© Ø§ÙÙÙØ§Ø·',    val:fmt(loyDisc)+' Ø¬',bg:'#fce7f3', tc:'#9d174d'},
    ...
    <div style="background:var(--bg-secondary);border-radius:8px;padding:12px;margin-bottom:14px;">
      <div style="font-size:13px;font-weight:700;margin-bottom:8px;">ð Ø£ÙØ«Ø± Ø§ÙÙÙØªØ¬Ø§Øª Ø´Ø±Ø§Ø¡Ù</div>
      <div style="display:flex;flex-wrap:wrap;gap:6px;">
        ${topProds.map(([name,qty],i)=>`<span style="background:white;border:1px solid var(--border);border-radius:20px;padding:3px 10px;font-size:12px;">${i+1}. ${name} <strong style="color:var(--primary);">Ã${qty}</strong></span>`).join('')}
      </div>
    </div>` : ''}

    ${c.notes ? `<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:10px;margin-bottom:14px;font-size:13px;">ð ${c.notes}</div>` : ''}

    <div style="font-size:13px;font-weight:700;margin-bottom:8px;">ð§¾ Ø¢Ø®Ø± Ø§ÙÙÙØ§ØªÙØ±</div>
    <div style="max-height:220px;overflow-y:auto;">
      ${allSales.slice(0,20).map(s=>{
        const d = new Date(s.date).toLocaleDateString('ar-EG');
        const t = new Date(s.date).toLocaleTimeString('ar-EG',{hour:'2-digit',minute:'2-digit'});
        const preview = (s.items||[]).slice(0,2).map(i=>i.name).join('Ø ');
        return `<div style="display:flex;justify-content:space-between;align-items:center;padding:8px;border-radius:6px;background:var(--bg-secondary);margin-bottom:4px;gap:8px;">
          ...
          <span style="font-weight:700;font-size:13px;flex-shrink:0;">${fmt(s.total)} Ø¬</span>
        </div>`;
      }).join('') || '<p style="text-align:center;color:var(--text-muted);padding:20px;">ÙØ§ ØªÙØ¬Ø¯ ÙÙØ§ØªÙØ±</p>'}
    </div>
  `;
...
// ââ Customer search in payment modal ââ
...
function searchCustomerDD() {
  ...
  _custDDTimer = setTimeout(() => {
    ...
    if (!results.length) {
      dd.innerHTML = '<div class="customer-dd-item" style="color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ ÙØªØ§Ø¦Ø¬</div>';
    } else {
      dd.innerHTML = results.map(c =>
        `<div class="customer-dd-item" onclick="selectCustomer('${c.id}')">
          <strong>${c.name}</strong>${c.phone ? ' â ' + c.phone : ''}
...
function selectCustomer(id) {
  ...
  document.getElementById('selectedCustomerName').textContent = c.name + (c.phone ? ' â '+c.phone : '');
...
function openReturnFromModal() {
  ...
  if (lastSaleForPrint.isReturn) { alert('ÙØ§ ÙÙÙÙ Ø¥Ø±Ø¬Ø§Ø¹ ÙØ§ØªÙØ±Ø© ÙØ±ØªØ¬Ø¹Ø©'); return; }
  ...
  document.getElementById('returnItemsList').innerHTML = lastSaleForPrint.items.map((item, idx) => `
    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 4px;border-bottom:1px solid var(--border);">
      <div style="flex:1;">
        ...
        <div style="font-size:11px;color:var(--text-muted);">Ø³Ø¹Ø± Ø§ÙÙØ­Ø¯Ø©: ${fmt(item.price)} Ø¬ Â· Ø§ÙÙÙÙØ© Ø§ÙØ£ØµÙÙØ©: ${item.qty}</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px;flex-shrink:0;">
        <label style="font-size:12px;color:var(--text-muted);">ÙÙÙØ© Ø§ÙØ¥Ø±Ø¬Ø§Ø¹:</label>
...
function processReturn() {
  ...
  if (!returnItems.length) { alert('Ø§Ø®ØªØ± Ø¹ÙÙ Ø§ÙØ£ÙÙ ØµÙÙ ÙØ§Ø­Ø¯ ÙÙØ¥Ø±Ø¬Ø§Ø¹'); return; }
  if (!confirm(`ØªØ£ÙÙØ¯ Ø¥Ø±Ø¬Ø§Ø¹ ${returnItems.length} ØµÙÙ â Ø§ÙÙØ¨ÙØº Ø§ÙÙØ³ØªØ±Ø¯: ${fmt(returnTotal)} Ø¬Ø`)) return;
  ...
  const returnSale = {
    ...
    returnReason:   reason || 'ØºÙØ± ÙØ­Ø¯Ø¯',
    cashier:        currentUser === 'admin' ? 'ÙØ¯ÙØ±' : 'ÙØ§Ø´ÙØ±',
  ...
  alert(`â ØªÙ ÙØ¹Ø§ÙØ¬Ø© Ø§ÙÙØ±ØªØ¬Ø¹ â Ø§ÙÙØ¨ÙØº Ø§ÙÙØ³ØªØ±Ø¯: ${fmt(returnTotal)} Ø¬\nð¦ ØªÙ Ø¥Ø¹Ø§Ø¯Ø© Ø§ÙÙÙÙØ§Øª ÙÙÙØ®Ø²ÙÙ`);
...
function buildReturnsReport() {
  ...
  document.getElementById('ret-total').textContent = fmt(total)+' Ø¬';
  document.getElementById('ret-units').textContent = units;
  document.getElementById('ret-body').innerHTML = !returns.length
    ? '<tr><td colspan="6" class="text-center text-muted" style="padding:24px;">ÙØ§ ØªÙØ¬Ø¯ ÙØ±ØªØ¬Ø¹Ø§Øª ÙÙ ÙØ°Ù Ø§ÙÙØªØ±Ø©</td></tr>'
    : returns.sort((a,b) => new Date(b.date)-new Date(a.date)).map(r => `<tr>
        <td><span class="badge-return">ÙØ±ØªØ¬Ø¹ #${String(r.id).slice(-6)}</span></td>
        ...
        <td>${r.items.map(i=>i.name+' Ã'+Math.abs(i.qty)).join(' Â· ')}</td>
        <td style="color:var(--danger);font-weight:700;">${fmt(Math.abs(r.total))} Ø¬</td>
...
const EXP_CATS = { rent:'Ø¥ÙØ¬Ø§Ø±', electricity:'ÙÙØ±Ø¨Ø§Ø¡', water:'ÙÙØ§Ù', internet:'Ø¥ÙØªØ±ÙØª/ØªÙÙÙÙÙ', salaries:'Ø±ÙØ§ØªØ¨', maintenance:'ØµÙØ§ÙØ©', marketing:'ØªØ³ÙÙÙ', transport:'ÙÙÙ/Ø´Ø­Ù', other:'Ø£Ø®Ø±Ù' };
const EXP_ICONS = { rent:'ð ', electricity:'â¡', water:'ð§', internet:'ð', salaries:'ð¤', maintenance:'ð§', marketing:'ð¢', transport:'ð', other:'ð' };
...
function openExpenseModal(id) {
  ...
  document.getElementById('expModalTitle').textContent = exp ? 'âï¸ ØªØ¹Ø¯ÙÙ ÙØµØ±ÙÙ' : 'â Ø¥Ø¶Ø§ÙØ© ÙØµØ±ÙÙ';
...
function saveExpense() {
  ...
  if (!amount || amount <= 0) { alert('Ø£Ø¯Ø®Ù ÙØ¨ÙØº ØµØ­ÙØ­'); return; }
  if (!date) { alert('Ø£Ø¯Ø®Ù Ø§ÙØªØ§Ø±ÙØ®'); return; }
  ...
  setExpenses(list);
  addAuditLog('expense.add', `ÙØµØ±ÙÙ: ${EXP_CATS[rec.category]} â ${fmt(rec.amount)} Ø¬ ${rec.type==='company'?'(Ø¥Ø¯Ø§Ø±Ù)':'('+(rec.branchId?getBranchName(rec.branchId):'')+')'}`, null);
...
function deleteExpense(id) {
  if (!confirm('Ø­Ø°Ù ÙØ°Ø§ Ø§ÙÙØµØ±ÙÙØ')) return;
...
function renderExpensesPage() {
  ...
  if (monthSel) {
    ...
    monthSel.innerHTML = '<option value="">Ø§ÙØ´ÙØ± Ø§ÙØ­Ø§ÙÙ</option>' + months.map(m=>`<option value="${m}" ${m===cur?'selected':''}>${m}</option>`).join('');
  }
  // Populate branch filter
  const brSel = document.getElementById('expBranchFilter');
  if (brSel && brSel.options.length <= 2) {
    BRANCH_IDS.forEach(b => {
      const o = document.createElement('option'); o.value=b; o.textContent='ð¬ '+getBranchName(b);
      brSel.appendChild(o);
    });
  }
  ...
  const el1=document.getElementById('exp-total');    if(el1) el1.textContent=fmt(totalAll)+' Ø¬';
  const el2=document.getElementById('exp-branches'); if(el2) el2.textContent=fmt(totalBr)+' Ø¬';
  const el3=document.getElementById('exp-company');  if(el3) el3.textContent=fmt(totalCo)+' Ø¬';
  const el4=document.getElementById('exp-net-profit'); if(el4) { el4.textContent=fmt(netProfit)+' Ø¬'; el4.style.color=netProfit>=0?'var(--success)':'var(--danger)'; }
  ...
  if (breakdown) {
    ...
    breakdown.innerHTML = BRANCH_IDS.map(b => {
      ...
      return `<div class="stat-card" style="border-top:3px solid var(--primary);">
        <div style="font-weight:700; font-size:13px; margin-bottom:6px;">ð¬ ${getBranchName(b)}</div>
        <div style="font-size:12px; color:var(--text-muted);">ÙØµØ§Ø±ÙÙ: <strong style="color:var(--danger);">${fmt(bExp)} Ø¬</strong></div>
        <div style="font-size:12px; color:var(--text-muted);">ÙØ¨ÙØ¹Ø§Øª: <strong style="color:var(--success);">${fmt(bSales)} Ø¬</strong></div>
      </div>`;
    }).join('');
  }
  ...
  if (!list.length) {
    tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted" style="padding:24px;">ÙØ§ ØªÙØ¬Ø¯ ÙØµØ§Ø±ÙÙ ÙÙØ°Ø§ Ø§ÙØ´ÙØ±</td></tr>'; return;
  }
  const sorted = [...list].sort((a,b)=>b.date.localeCompare(a.date));
  tbody.innerHTML = sorted.map(e => `<tr>
    <td style="white-space:nowrap; font-size:12px;">${e.date}</td>
    <td><span style="background:${e.type==='company'?'#f3e8ff':'#dbeafe'};color:${e.type==='company'?'#7c3aed':'#1d4ed8'};padding:2px 8px;border-radius:10px;font-size:11px;">${e.type==='company'?'ð¢ Ø¥Ø¯Ø§Ø±Ù':'ð¬ ÙØ±Ø¹'}</span></td>
    <td style="font-size:12px;">${e.type==='company'?'Ø´Ø±ÙØ©':getBranchName(e.branchId)}</td>
    <td><span style="font-size:12px;">${EXP_ICONS[e.category]||''} ${EXP_CATS[e.category]||e.category}</span></td>
    <td style="font-weight:700; color:var(--danger);">${fmt(e.amount)} Ø¬</td>
    <td style="font-size:12px; color:var(--text-muted);">${e.note||'-'}</td>
    <td style="font-size:11px; color:var(--text-muted);">${e.by||''}</td>
    <td>
      <button class="btn btn-sm" onclick="openExpenseModal('${e.id}')" style="font-size:11px;padding:3px 8px;margin-left:4px;">âï¸</button>
      <button class="btn btn-danger btn-sm" onclick="deleteExpense('${e.id}')" style="font-size:11px;padding:3px 8px;">ðï¸</button>
    </td>
  </tr>`).join('');
}
...
function renderAuditPage() {
  ...
  const actionLabels = {
    'inv.add':'â Ø¥Ø¶Ø§ÙØ© ØµÙÙ', 'inv.edit':'âï¸ ØªØ¹Ø¯ÙÙ ØµÙÙ', 'inv.delete':'ðï¸ Ø­Ø°Ù ØµÙÙ',
    'price.change':'ð·ï¸ ØªØºÙÙØ± Ø³Ø¹Ø±', 'sale.complete':'ð° ÙØ§ØªÙØ±Ø© ÙÙØªÙÙØ©', 'sale.return':'â©ï¸ ÙØ±ØªØ¬Ø¹',
    'po.receive':'ð¦ Ø§Ø³ØªÙØ§Ù Ø¨Ø¶Ø§Ø¹Ø©', 'po.create':'ð Ø£ÙØ± Ø´Ø±Ø§Ø¡', 'transfer.done':'ð ØªØ­ÙÙÙ',
    'expense.add':'ð¸ ÙØµØ±ÙÙ', 'settings.change':'âï¸ ØªØºÙÙØ± Ø¥Ø¹Ø¯Ø§Ø¯',
    'auth.login':'ð ØªØ³Ø¬ÙÙ Ø¯Ø®ÙÙ', 'auth.logout':'ðª ØªØ³Ø¬ÙÙ Ø®Ø±ÙØ¬'
  };

  const tbody = document.getElementById('auditBody'); if(!tbody) return;
  if (!list.length) {
    tbody.innerHTML = '<tr><td colspan="5" class="text-center text-muted" style="padding:24px;">ÙØ§ ØªÙØ¬Ø¯ Ø³Ø¬ÙØ§Øª</td></tr>'; return;
  }
...
function shareReceiptWhatsApp() {
  ...
  if (!sale) { alert('ÙØ§ ØªÙØ¬Ø¯ ÙØ§ØªÙØ±Ø© ÙÙØ´Ø§Ø±ÙØªÙØ§'); return; }
  const lines = [
    `ð§¾ ÙØ§ØªÙØ±Ø© ÙÙ VOODO ERP`,
    `ð ${new Date(sale.date).toLocaleString('ar-EG')}`,
    `ð¬ ${sale.branchName || getBranchName(sale.branchId || 'b1')}`,
    `ð¤ ${sale.salesperson || sale.cashier || ''}`,
    `ââââââââââââââââââââ`,
    ...sale.items.map(i => `â¢ ${i.name} Ã${i.qty}  =  ${fmt(i.price * i.qty)} Ø¬`),
    `ââââââââââââââââââââ`,
    sale.disc > 0 ? `ð Ø®ØµÙ: ${fmt(sale.disc)} Ø¬` : '',
    `ð° Ø§ÙØ¥Ø¬ÙØ§ÙÙ: ${fmt(sale.total)} Ø¬`,
    `ð³ Ø§ÙØ¯ÙØ¹: ${sale.payMethod === 'cash' ? 'ÙÙØ¯Ù' : 'ÙØ§Ø±Øª'}`,
    ``,
    `Ø´ÙØ±Ø§Ù ÙØ«ÙØªÙ Ø¨Ù VOODO ERP ð `
...
function shareOffersWhatsApp() {
  ...
  if (!promos.length) { alert('ÙØ§ ØªÙØ¬Ø¯ Ø¹Ø±ÙØ¶ ÙØ´Ø·Ø© ÙÙÙØ´Ø§Ø±ÙØ©'); return; }
  const lines = [
    `ð·ï¸ *Ø¹Ø±ÙØ¶ ÙØ­Ø²Ù VOODO ERP*`,
    `ââââââââââââââââââââ`,
    ...promos.map(p => {
      if (p.type === 'bundle') {
        return `ð¦ *${p.name}*\nØ§Ø´ØªØ±Ù: ${p.items.map(i=>i.name+' Ã'+i.minQty).join(' + ')}\nØ¨Ø³Ø¹Ø±: ${fmt(p.bundlePrice)} Ø¬ Ø¨Ø¯Ù ${fmt(p.items.reduce((s,i)=>{const inv=getInv().find(x=>x.code===i.code);return s+(inv?.priceAfter||inv?.price||0)*i.minQty;},0))} Ø¬`;
      } else {
        return `ð° *${p.name}*\nØ¹ÙØ¯ Ø´Ø±Ø§Ø¡ Ø¨Ø£ÙØ«Ø± ÙÙ ${fmt(p.minAmount)} Ø¬\nØ®ØµÙ ${p.discountType==='percent'?p.discountValue+'%':fmt(p.discountValue)+' Ø¬'}`;
      }
    }),
    `ââââââââââââââââââââ`,
    `ð  VOODO ERP â Ø¨Ø§Ø¯Ø± Ø¨Ø§ÙØ´Ø±Ø§Ø¡!`
  ];
  const text = encodeURIComponent(lines.join('\n'));
  window.open(`https://wa.me/?text=${text}`, '_blank');
}
...
function openCashierReturn() {
  ...
  document.getElementById('crResultsBody').innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:20px;">Ø§ÙØªØ¨ Ø±ÙÙ Ø§ÙØªÙÙÙÙÙ Ø£Ù Ø±ÙÙ Ø§ÙÙØ§ØªÙØ±Ø© Ø£Ù Ø§Ø®ØªØ± ØªØ§Ø±ÙØ® ÙÙØ¨Ø­Ø«</p>';
...
function searchSalesForReturn() {
  ...
  if (!q && !dStr) {
    body.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:20px;">Ø§ÙØªØ¨ Ø±ÙÙ Ø§ÙØªÙÙÙÙÙ Ø£Ù Ø±ÙÙ Ø§ÙÙØ§ØªÙØ±Ø© Ø£Ù Ø§Ø®ØªØ± ØªØ§Ø±ÙØ® ÙÙØ¨Ø­Ø«</p>';
    return;
  }
  const allSales = getSales().filter(s => !s.isReturn);
  ...
  if (!results.length) {
    body.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:20px;">ÙØ§ ØªÙØ¬Ø¯ ÙØªØ§Ø¦Ø¬</p>';
    return;
  }
  ...
  body.innerHTML = results.map(s => {
    const d = new Date(s.date).toLocaleDateString('ar-EG');
    const t = new Date(s.date).toLocaleTimeString('ar-EG', {hour:'2-digit', minute:'2-digit'});
    const itemsPreview = (s.items||[]).slice(0,2).map(i=>i.name).join('Ø ') + ((s.items||[]).length>2?' ...':'');
    ...
    return `<div onclick="${alreadyReturned ? '' : `openCashierReturnItems(${s.id})`}"
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">
        <div>
          ...
          ${s.customerName?`<span style="margin-inline-start:8px;font-size:12px;color:var(--text-muted);">ð¤ ${s.customerName}</span>`:''}
          ${(s.customerPhone||s.phone)?`<span style="margin-inline-start:8px;font-size:12px;color:var(--text-muted);">ð ${s.customerPhone||s.phone}</span>`:''}
          ${alreadyReturned?'<span style="margin-inline-start:8px;font-size:11px;color:var(--danger);font-weight:600;">â ØªÙ Ø§ÙØ¥Ø±Ø¬Ø§Ø¹</span>':''}
        </div>
        <div style="text-align:end;flex-shrink:0;">
          <div style="font-weight:700;color:var(--primary);">${fmt(s.total)} Ø¬</div>
...
function openCashierReturnItems(saleId) {
  ...
  document.getElementById('crInvoiceInfo').innerHTML = `
    <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:6px;">
      <div><b>ÙØ§ØªÙØ±Ø© #${String(sale.id).slice(-6)}</b> Â· ${d}</div>
      <div style="color:var(--primary);font-weight:700;">${fmt(sale.total)} Ø¬</div>
    </div>
    ${sale.customerName?`<div style="margin-top:4px;">ð¤ ${sale.customerName}${(sale.customerPhone||sale.phone)?' Â· ð '+(sale.customerPhone||sale.phone):''}</div>`:''}`;
  document.getElementById('crItemsList').innerHTML = sale.items.map((item, idx) => `
    <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 4px;border-bottom:1px solid var(--border);">
      <div style="flex:1;">
        <div style="font-size:13px;font-weight:600;">${item.name}</div>
        <div style="font-size:11px;color:var(--text-muted);">ÙÙØ¯: ${item.code} Â· ${fmt(item.price)} Ø¬ Ã ${item.qty}</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px;flex-shrink:0;">
        <label style="font-size:12px;color:var(--text-muted);">Ø¥Ø±Ø¬Ø§Ø¹:</label>
        ...
        <span style="font-size:11px;color:var(--text-muted);">ÙÙ ${item.qty}</span>
      </div>
    </div>`).join('');
...
function processCashierReturn() {
  ...
  addAuditLog('return', `ÙØ±ØªØ¬Ø¹ ÙÙ ÙØ§ØªÙØ±Ø© #${String(sale.id).slice(-6)} â ${fmt(returnTotal)} Ø¬`, currentBranch);
...
function sendReturnWhatsApp() {
  ...
  if (!s) return;
  const lines = [
    `ð Ø¥ÙØµØ§Ù ÙØ±ØªØ¬Ø¹ â VOODO ERP`,
    `ð ${new Date(s.date).toLocaleString('ar-EG')}`,
    `ð¬ ${s.branchName || getBranchName(s.branchId || 'b1')}`,
    `ÙØ§ØªÙØ±Ø© Ø£ØµÙÙØ©: #${String(s.originalSaleId).slice(-6)}`,
    `ââââââââââââââââââââ`,
    ...s.items.map(i => `â¢ ${i.name} Ã${Math.abs(i.qty)}  =  ${fmt(Math.abs(i.qty)*i.price)} Ø¬`),
    `ââââââââââââââââââââ`,
    `ð° Ø§ÙÙØ¨ÙØº Ø§ÙÙØ³ØªØ±Ø¯: ${fmt(Math.abs(s.total))} Ø¬`,
    s.returnReason !== 'ØºÙØ± ÙØ­Ø¯Ø¯' ? `ð Ø§ÙØ³Ø¨Ø¨: ${s.returnReason}` : '',
    ``,
    `Ø´ÙØ±Ø§Ù ÙØ«ÙØªÙ Ø¨Ù VOODO ERP ð `
  ].filter(l => l !== '');
  const text = encodeURIComponent(lines.join('\n'));
...
function renderBarcodeList() {
  ...
  container.innerHTML = inv.map(i => `<div style="display:flex; align-items:center; gap:8px; padding:5px 4px; border-bottom:1px solid var(--border); font-size:12px;">
    ...
    <span style="color:var(--primary); font-weight:600;">${fmt(i.priceAfter)} Ø¬</span>
  </div>`).join('');
}
...
function buildPriceTag(item, showPrice, dim, preview) {
  const priceHTML = showPrice === 'none' ? '' :
    showPrice === 'after' ? `<div style="font-size:${preview?'11px':'14px'}; font-weight:800; color:#1a5faf; margin-top:2px;">${fmt(item.priceAfter)} Ø¬</div>` :
    showPrice === 'before' ? `<div style="font-size:${preview?'11px':'14px'}; font-weight:800; color:#1a5faf; margin-top:2px;">${fmt(item.priceBefore||item.priceAfter)} Ø¬</div>` :
    `<div style="font-size:9px; color:#888; text-decoration:line-through;">${fmt(item.priceBefore||item.priceAfter)} Ø¬</div><div style="font-size:${preview?'12px':'16px'}; font-weight:800; color:#1a5faf;">${fmt(item.priceAfter)} Ø¬</div>`;
...
function printBarcodes() {
  const inv = getInv().filter(i => _bcSelected.has(i.code));
  if (!inv.length) { alert('Ø§Ø®ØªØ± Ø£ØµÙØ§Ù Ø£ÙÙØ§Ù'); return; }
  const copies = parseInt(document.getElementById('bcCopies')?.value) || 1;
  const showPrice = document.getElementById('bcShowPrice')?.value || 'after';
  const dim = getBCTagDimensions();
  let tags = [];
...
function saveLoyaltySettings() {
  ...
  alert('ØªÙ Ø­ÙØ¸ Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª Ø§ÙÙÙØ§Ø¡ â');
}
// ââââââââââââââââââââââââââââââââââââââââââââââ
...
function renderAttendancePane() {
  ...
  const SL = {present:'â',absent:'â',late:'â°',excused:'ðµ','':'â'};
  ...
  const summCards = sps.map(name => {
    ...
    return `<div style="background:var(--bg-secondary);border-radius:8px;padding:10px 14px;display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
      ...
      <span style="background:#dcfce7;color:#15803d;padding:2px 8px;border-radius:12px;font-size:12px;">â ${p} Ø­Ø¶ÙØ±</span>
      <span style="background:#fee2e2;color:#b91c1c;padding:2px 8px;border-radius:12px;font-size:12px;">â ${ab} ØºÙØ§Ø¨</span>
      <span style="background:#fef9c3;color:#854d0e;padding:2px 8px;border-radius:12px;font-size:12px;">â° ${lt} ØªØ£Ø®ÙØ±</span>
    </div>`;
  }).join('');
  ...
  const rows = recentDays.map(ds => {
    const cells = sps.map(name => {
      ...
      return `<td style="background:${SC[st]};text-align:center;cursor:pointer;padding:8px 6px;border:1px solid var(--border);"
        title="${rec?.checkIn||''} â ${rec?.checkOut||''}" onclick="openAttendanceEdit('${name}','${ds}')">${SL[st]}</td>`;
    }).join('');
    return `<tr><td style="font-size:12px;color:var(--text-muted);padding:6px 8px;white-space:nowrap;border:1px solid var(--border);">${ds.slice(5)}${ds===today?' ðµ':''}</td>${cells}</tr>`;
  }).join('');
  ...
  pane.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;flex-wrap:wrap;gap:8px;">
      <div style="font-weight:700;font-size:14px;">ÙÙØ®Øµ ${month}</div>
      <button class="btn btn-primary btn-sm" onclick="openAttendanceEdit('','${today}')">â ØªØ³Ø¬ÙÙ Ø­Ø¶ÙØ±</button>
    </div>
    <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:16px;">${summCards}</div>
    <div style="overflow-x:auto;">
      <table style="width:100%;border-collapse:collapse;font-size:13px;">
        <thead><tr>
          <th style="padding:6px 8px;background:var(--bg-secondary);border:1px solid var(--border);">Ø§ÙØªØ§Ø±ÙØ®</th>
...
function saveAttendanceEdit() {
  ...
  if (!emp||!date) { alert('Ø§Ø®ØªØ± Ø§ÙÙÙØ¸Ù ÙØ§ÙØªØ§Ø±ÙØ®'); return; }
...
function renderPayrollPane() {
  ...
  if (!pane) return;
  if (!payroll.length) { pane.innerHTML='<p style="text-align:center;color:var(--text-muted);padding:30px;">ÙØ§ ÙÙØ¬Ø¯ Ø¨Ø§Ø¦Ø¹ÙÙ â Ø£Ø¶ÙÙÙ ÙÙ Ø§ÙØ¥Ø¹Ø¯Ø§Ø¯Ø§Øª</p>'; return; }
  ...
  pane.innerHTML = `
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px;">
      ${[{l:'Ø¥Ø¬ÙØ§ÙÙ Ø§ÙØ±ÙØ§ØªØ¨',v:fmt(totalNet)+' Ø¬',bg:'#eff6ff',tc:'#1d4ed8'},{l:'ØªÙ Ø§ÙØµØ±Ù',v:fmt(totalPaid)+' Ø¬',bg:'#dcfce7',tc:'#15803d'},{l:'ÙØªØ¨ÙÙ ÙÙØµØ±Ù',v:fmt(totalNet-totalPaid)+' Ø¬',bg:'#fee2e2',tc:'#b91c1c'}]
    ...
    <table style="width:100%;border-collapse:collapse;font-size:13px;min-width:700px;">
      <thead><tr style="background:var(--bg-secondary);">
        ${['Ø§ÙÙÙØ¸Ù','Ø§ÙÙØ¨ÙØ¹Ø§Øª','Ø¹ÙÙÙØ©%','Ø§ÙØ¹ÙÙÙØ©','Ø±Ø§ØªØ¨ Ø£Ø³Ø§Ø³Ù','ÙÙØ§ÙØ£Ø©','Ø®ØµÙ','Ø§ÙØµØ§ÙÙ','Ø§ÙØ­Ø§ÙØ©','Ø¥Ø¬Ø±Ø§Ø¡']
      ...
      <tbody>
        ${payroll.map(p=>`<tr style="border-bottom:1px solid var(--border);">
          ...
          <td style="padding:8px;text-align:center;">${fmt(p.empSales)} Ø¬</td>
          <td style="padding:8px;text-align:center;">${p.commPct}%</td>
          <td style="padding:8px;text-align:center;color:#059669;font-weight:700;">${fmt(p.commission)} Ø¬</td>
          <td style="padding:8px;text-align:center;">${fmt(p.base)} Ø¬</td>
          ...
          <td style="padding:8px;text-align:center;font-weight:800;color:var(--primary);font-size:14px;">${fmt(p.net)} Ø¬</td>
          <td style="padding:8px;text-align:center;">
            <span style="background:${p.isPaid?'#dcfce7':'#fee2e2'};color:${p.isPaid?'#15803d':'#b91c1c'};padding:2px 8px;border-radius:12px;font-size:11px;">${p.isPaid?'â ÙØµØ±ÙÙ':'â³ ÙÙ ÙÙØµØ±Ù'}</span>
          </td>
          <td style="padding:8px;text-align:center;">
            <button class="btn btn-sm" style="background:${p.isPaid?'#fee2e2':'#dcfce7'};color:${p.isPaid?'#b91c1c':'#15803d'};"
              ...
              ${p.isPaid?'â© Ø§Ø³ØªØ±Ø¯Ø§Ø¯':'ðµ ØµØ±Ù Ø§ÙØ±Ø§ØªØ¨'}
            </button>
          </td>
        </tr>`).join('')}
      </tbody>
...
// ââââââââââââââââââââââââââââââââââââââââââââââ
// ACCOUNTING â ÙØ­Ø§Ø³Ø¨Ø© Ø±Ø³ÙÙØ©
// ââââââââââââââââââââââââââââââââââââââââââââââ
let _accTab = 'pnl';
...
function renderPnL() {
  ...
  const row = (label, amt, indent=0, bold=false, sep=false, color='') => {
    ...
    return `<tr><td style="${st}">${label}</td>
      <td style="${st}text-align:left;direction:ltr;">${amt!==null?(amt<0?'-':'')+fmt(Math.abs(amt))+' Ø¬':''}</td>
  ...
  pane.innerHTML = `<div style="max-width:580px;">
    <div style="font-size:15px;font-weight:800;margin-bottom:14px;color:var(--primary);">ð ÙØ§Ø¦ÙØ© Ø§ÙØ¯Ø®Ù â ${month}</div>
    <table style="width:100%;border-collapse:collapse;font-size:13px;background:white;border-radius:10px;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,.07);">
      <thead><tr style="background:var(--sidebar);color:white;">
        <th style="padding:10px 8px;text-align:right;">Ø§ÙØ¨ÙØ¯</th>
        <th style="padding:10px 8px;text-align:left;">Ø§ÙÙØ¨ÙØº</th>
        <th style="padding:10px 8px;text-align:center;">%</th>
      </tr></thead><tbody>
      ${row('Ø¥ÙØ±Ø§Ø¯Ø§Øª Ø§ÙÙØ¨ÙØ¹Ø§Øª',revenue,0,true,false,'#059669')}
      ${row('Ø§ÙÙØ±ØªØ¬Ø¹Ø§Øª',-returnAmt,1)}
      ${row('ØµØ§ÙÙ Ø§ÙØ¥ÙØ±Ø§Ø¯',netRev,0,true,true,'#1d4ed8')}
      ${row('ØªÙÙÙØ© Ø§ÙØ¨Ø¶Ø§Ø¹Ø© Ø§ÙÙØ¨Ø§Ø¹Ø©',-cogs,1)}
      ${row('Ø¥Ø¬ÙØ§ÙÙ Ø§ÙØ±Ø¨Ø­',grossP,0,true,true,grossP>=0?'#059669':'#dc2626')}
      <tr><td colspan="3" style="padding:4px 8px;background:var(--bg-secondary);font-size:11px;font-weight:700;color:var(--text-muted);">Ø§ÙÙØµØ§Ø±ÙÙ Ø§ÙØªØ´ØºÙÙÙØ©</td></tr>
      ${row('Ø§ÙØ±ÙØ§ØªØ¨',-salaries,1)}
      ${row('ÙØµØ§Ø±ÙÙ Ø£Ø®Ø±Ù',-opExp,1)}
      ${row('Ø¥Ø¬ÙØ§ÙÙ Ø§ÙÙØµØ§Ø±ÙÙ',-totalOp,0,true,true)}
      ${row('Ø§ÙØ±Ø¨Ø­ Ø§ÙØªØ´ØºÙÙÙ (EBIT)',ebit,0,true,true,ebit>=0?'#059669':'#dc2626')}
      </tbody>
    </table>
    <div style="margin-top:12px;display:grid;grid-template-columns:1fr 1fr;gap:10px;">
      ${[{l:'ÙØ§ÙØ´ Ø§ÙØ±Ø¨Ø­ Ø§ÙØ¥Ø¬ÙØ§ÙÙ',v:gMargin+'%',bg:'#dcfce7',tc:'#15803d'},{l:'ØµØ§ÙÙ Ø§ÙØ±Ø¨Ø­',v:fmt(ebit)+' Ø¬',bg:ebit>=0?'#dcfce7':'#fee2e2',tc:ebit>=0?'#15803d':'#b91c1c'}]
...
function renderCashFlow() {
  ...
  const cashIn  = (byM['cash']||0)+(byM['ÙÙØ¯Ù']||0);
  const cardIn  = (byM['card']||0)+(byM['ÙÙØ²Ø§']||0)+(byM['ÙØ±ÙØ¯Øª']||0);
  const otherIn = Object.entries(byM).filter(([k])=>!['cash','ÙÙØ¯Ù','card','ÙÙØ²Ø§','ÙØ±ÙØ¯Øª'].includes(k)).reduce((s,[,v])=>s+v,0);
  ...
  const bars = daily.map(({d,s,e})=>`<div style="display:flex;flex-direction:column;align-items:center;gap:1px;flex:1;min-width:6px;">
    <div style="width:60%;background:#1a5faf;height:${Math.round(s/mx*70)}px;border-radius:2px 2px 0 0;" title="ÙØ¨ÙØ¹Ø§Øª ${fmt(s)} Ø¬"></div>
    <div style="width:60%;background:#ef4444;height:${Math.round(e/mx*70)}px;border-radius:2px 2px 0 0;" title="ÙØµØ§Ø±ÙÙ ${fmt(e)} Ø¬"></div>
    ${days<=16?`<div style="font-size:8px;color:var(--text-muted);">${d}</div>`:''}
  </div>`).join('');

  pane.innerHTML = `
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px;">
      ${[{l:'Ø¥Ø¬ÙØ§ÙÙ Ø§ÙØ¯Ø§Ø®Ù',v:'+'+fmt(totalIn)+' Ø¬',bg:'#dcfce7',tc:'#15803d'},{l:'Ø¥Ø¬ÙØ§ÙÙ Ø§ÙØ®Ø§Ø±Ø¬',v:'-'+fmt(totalOut)+' Ø¬',bg:'#fee2e2',tc:'#b91c1c'},{l:'ØµØ§ÙÙ Ø§ÙØªØ¯ÙÙ',v:(net>=0?'+':'')+fmt(net)+' Ø¬',bg:net>=0?'#eff6ff':'#fef3c7',tc:net>=0?'#1d4ed8':'#854d0e'}]
    ...
    <div style="background:white;border-radius:10px;padding:14px;margin-bottom:16px;box-shadow:0 1px 6px rgba(0,0,0,.07);">
      <div style="font-weight:700;margin-bottom:10px;font-size:13px;">ð <span style="color:#1a5faf;">â  ÙØ¨ÙØ¹Ø§Øª</span> <span style="color:#ef4444;">â  ÙØµØ§Ø±ÙÙ</span></div>
    ...
    <div style="background:white;border-radius:10px;padding:14px;box-shadow:0 1px 6px rgba(0,0,0,.07);">
      <table style="width:100%;font-size:13px;border-collapse:collapse;">
        ${[['ð¥ ÙØ¨ÙØ¹Ø§Øª ÙØ§Ø´','+'+fmt(cashIn)+' Ø¬','#059669'],['ð³ ÙØ¨ÙØ¹Ø§Øª Ø¨Ø·Ø§ÙØ©','+'+fmt(cardIn)+' Ø¬','#059669'],['ð¥ Ø£Ø®Ø±Ù','+'+fmt(otherIn)+' Ø¬','#059669'],['ð¤ ÙØ±ØªØ¬Ø¹Ø§Øª','-'+fmt(retOut)+' Ø¬','#dc2626'],['ð¤ ÙØµØ§Ø±ÙÙ','-'+fmt(expOut)+' Ø¬','#dc2626'],['ð¤ ÙÙØ±Ø¯ÙÙ','-'+fmt(purOut)+' Ø¬','#dc2626']]
        ...
        <tr style="background:var(--bg-secondary);"><td style="padding:10px;font-weight:800;">ð¦ ØµØ§ÙÙ Ø§ÙØªØ¯ÙÙ</td><td style="padding:10px;font-weight:800;text-align:left;direction:ltr;color:${net>=0?'#15803d':'#dc2626'};font-size:15px;">${(net>=0?'+':'')+fmt(net)} Ø¬</td></tr>
      </table>
    </div>`;
}
...
function renderAccSummary() {
  ...
  const rows = months.slice(0,12).map(m=>{
    ...
    return `<tr style="border-bottom:1px solid var(--border);">
      ...
      <td style="padding:8px;text-align:center;">${fmt(mS)} Ø¬</td>
      <td style="padding:8px;text-align:center;">${fmt(mC)} Ø¬</td>
      <td style="padding:8px;text-align:center;">${fmt(mE)} Ø¬</td>
      <td style="padding:8px;text-align:center;font-weight:700;color:${mP>=0?'#059669':'#dc2626'};">${fmt(mP)} Ø¬</td>
    </tr>`;
  }).join('');

  pane.innerHTML = `
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:20px;">
      ${[{l:'Ø¥Ø¬ÙØ§ÙÙ Ø§ÙØ¥ÙØ±Ø§Ø¯',v:fmt(totalRev)+' Ø¬',bg:'#eff6ff',tc:'#1d4ed8'},{l:'Ø¥Ø¬ÙØ§ÙÙ Ø§ÙØªÙØ§ÙÙÙ',v:fmt(totalCOGS)+' Ø¬',bg:'#fef9c3',tc:'#854d0e'},{l:'Ø¥Ø¬ÙØ§ÙÙ Ø§ÙÙØµØ§Ø±ÙÙ',v:fmt(totalExp)+' Ø¬',bg:'#fee2e2',tc:'#b91c1c'},{l:'ØµØ§ÙÙ Ø§ÙØ£Ø±Ø¨Ø§Ø­ Ø§ÙÙÙÙ',v:fmt(totalNP)+' Ø¬',bg:totalNP>=0?'#dcfce7':'#fee2e2',tc:totalNP>=0?'#15803d':'#b91c1c'}]
    ...
    <div style="background:white;border-radius:10px;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,.07);">
      <table style="width:100%;border-collapse:collapse;font-size:13px;">
        <thead><tr style="background:var(--sidebar);color:white;">
          ${['Ø§ÙØ´ÙØ±','Ø§ÙØ¥ÙØ±Ø§Ø¯','Ø§ÙØªÙÙÙØ©','Ø§ÙÙØµØ§Ø±ÙÙ','ØµØ§ÙÙ Ø§ÙØ±Ø¨Ø­'].map(h=>`<th style="padding:10px 8px;text-align:center;">${h}</th>`).join('')}
        </tr></thead>
        <tbody>${rows||'<tr><td colspan="5" style="text-align:center;padding:20px;color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ Ø¨ÙØ§ÙØ§Øª</td></tr>'}</tbody>
      </table>
    </div>`;
}

// ââââââââââââââââââââââââââââââââââââââââââââââ
// VLOOKUP â ØªÙØ§Ø±ÙØ± Ø±Ø¨Ø· Ø§ÙØ¬Ø¯Ø§ÙÙ
// ââââââââââââââââââââââââââââââââââââââââââââââ
let _vlTab = 'supplier';
...
// ââ Category / Family Sales Report ââââââââââââââââââââââââââââââââ
function renderCategoryReport(fromDate, toDate) {
  ...
  allSales.forEach(function(sale) {
    (sale.items||[]).forEach(function(item) {
      ...
      var cat  = (prod && prod.category) ? prod.category : (item.category || 'ØºÙØ± ÙØ­Ø¯Ø¯');
      var fam  = (prod && prod.family)   ? prod.family   : (item.family   || 'ØºÙØ± ÙØ­Ø¯Ø¯');
  ...
  function buildTable(map, title, icon) {
    ...
    if (!rows.length) return '<div style="padding:20px;color:var(--text-muted);text-align:center;">ÙØ§ ØªÙØ¬Ø¯ ÙØ¨ÙØ¹Ø§Øª ÙÙ ÙØ°Ù Ø§ÙÙØªØ±Ø©</div>';
    ...
    ['Ø§ÙØ§Ø³Ù','Ø§ÙØ¥ÙØ±Ø§Ø¯','Ø§ÙØªÙÙÙØ©','ÙØ§ÙØ´ %','Ø§ÙÙÙÙØ©','% ÙÙ Ø§ÙØ¥Ø¬ÙØ§ÙÙ','Ø£Ø¹ÙÙ ÙÙØªØ¬'].forEach(function(h){ t += '<th style="padding:9px 8px;text-align:center;">'+h+'</th>'; });
    t += '</tr></thead><tbody>';
    rows.forEach(function(r,i) {
      ...
      t += '<td style="padding:8px;text-align:center;font-weight:700;color:var(--primary);">'+fmt(r.revenue)+' Ø¬</td>';
      t += '<td style="padding:8px;text-align:center;">'+fmt(r.cost)+' Ø¬</td>';
  ...
  [{l:'Ø¥ÙØ±Ø§Ø¯ Ø§ÙÙØªØ±Ø©', v:fmt(totalRev)+' Ø¬', bg_l:'#eff6ff', tc_l:'#1d4ed8', bg_d:'#0d1e35', tc_d:'#60a5fa'},
   {l:'ÙØ§ÙØ´ Ø§ÙØ±Ø¨Ø­',   v:totalMargin+'%',    bg_l:'#dcfce7', tc_l:'#15803d', bg_d:'#0a2218', tc_d:'#4ade80'},
   {l:'Ø¹Ø¯Ø¯ Ø§ÙÙÙØ§ØªÙØ±', v:salesCount,         bg_l:'#f5f3ff', tc_l:'#6d28d9', bg_d:'#1a1035', tc_d:'#a78bfa'},
   {l:'Ø¹Ø¯Ø¯ Ø§ÙÙØ¦Ø§Øª',   v:catCount,           bg_l:'#fef9c3', tc_l:'#854d0e', bg_d:'#2d1f06', tc_d:'#fbbf24'},
   {l:'Ø¹Ø¯Ø¯ Ø§ÙÙØ¬ÙÙØ¹Ø§Øª',v:famCount,           bg_l:'#fde8d0', tc_l:'#9a3412', bg_d:'#2a1205', tc_d:'#fb923c'}].forEach(function(k){
  ...
  out += '</div>';
  out += buildTable(catMap, 'ØªÙØ±ÙØ± Ø­Ø³Ø¨ Ø§ÙÙØ¦Ø© (Category)', 'ð·ï¸');
  out += buildTable(famMap, 'ØªÙØ±ÙØ± Ø­Ø³Ø¨ Ø§ÙÙØ¬ÙÙØ¹Ø© (Family)', 'ð¦');
  pane.innerHTML = out;
}
...
function openBulkCategoryModal() {
  ...
  inv.forEach(function(p,i) {
    var catOpts = cats.map(function(v){ return '<option value="'+v+'" '+(((p.category||'')===(v))?'selected':'')+'>'+( v||'â Ø§Ø®ØªØ± Ø§ÙÙØ¦Ø© â')+'</option>'; }).join('');
    var famOpts = fams.map(function(v){ return '<option value="'+v+'" '+(((p.family||'')===(v))?'selected':'')+'>'+( v||'â Ø§Ø®ØªØ± Ø§ÙÙØ¬ÙÙØ¹Ø© â')+'</option>'; }).join('');
...
function saveBulkCategories() {
  ...
  alert(changed > 0 ? ('ØªÙ ØªØ­Ø¯ÙØ« ' + changed + ' ÙÙØªØ¬') : 'ÙØ§ ÙÙØ¬Ø¯ ØªØºÙÙØ±Ø§Øª');
}
...
function handleVlFile(input) {
  ...
  reader.onload = function(e) {
    try {
      ...
      if (!rows.length) { alert('Ø§ÙÙÙÙ ÙØ§Ø±Øº Ø£Ù ØºÙØ± ÙÙØ±ÙØ¡'); return; }
      _vlData    = rows;
      _vlHeaders = Object.keys(rows[0]);
      buildVlMappingUI();
    } catch(ex) { alert('Ø®Ø·Ø£ ÙÙ ÙØ±Ø§Ø¡Ø© Ø§ÙÙÙÙ: ' + ex.message); }
  };
  reader.readAsBinaryString(file);
}
...
function buildVlMappingUI() {
  ...
  var opts = '<option value="">â Ø§Ø®ØªØ± Ø¹ÙÙØ¯ â</option>' + _vlHeaders.map(function(h){ return '<option value="'+h+'">'+h+'</option>'; }).join('');
  var optsWithSkip = '<option value="">â ÙØ§ ØªØ³ØªÙØ±Ø¯</option>' + _vlHeaders.map(function(h){ return '<option value="'+h+'">'+h+'</option>'; }).join('');
  ...
  var autoMatch = {
    vlKeyCol:         ['code','ÙÙØ¯','Ø§ÙÙÙØ¯','sku','barcode','item code','ÙÙØ¯ Ø§ÙÙÙØªØ¬'],
    vlCostCol:        ['cost','ØªÙÙÙØ©','Ø§ÙØªÙÙÙØ©','buying price','buy price','Ø³Ø¹Ø± Ø§ÙØ´Ø±Ø§Ø¡'],
    vlPriceBeforeCol: ['price before','pricebefore','Ø§ÙØ³Ø¹Ø± ÙØ¨Ù','ÙØ¨Ù','old price','before'],
    vlPriceAfterCol:  ['price after','priceafter','Ø§ÙØ³Ø¹Ø± Ø¨Ø¹Ø¯','Ø¨Ø¹Ø¯','price','Ø§ÙØ³Ø¹Ø±','after'],
    vlCategoryCol:    ['category','Ø§ÙÙØ¦Ø©','ÙØ¦Ø©','ÙØ§ØªÙØ¬ÙØ±Ù','ÙØ§ØªÙØ¬ÙØ±Ù','cat'],
    vlFamilyCol:      ['family','Ø§ÙÙØ¬ÙÙØ¹Ø©','ÙØ¬ÙÙØ¹Ø©','ÙØ§ÙÙÙÙ','ÙØ§ÙÙÙÙ','fam']
  };
  ...
  document.getElementById('vlFileInfo').textContent = _vlData.length + ' ØµÙ â¢ ' + _vlHeaders.length + ' Ø¹ÙÙØ¯';
}

function previewVlookup() {
  var keyCol = document.getElementById('vlKeyCol').value;
  if (!keyCol) { alert('Ø§Ø®ØªØ± Ø¹ÙÙØ¯ Ø§ÙÙÙØ¯ Ø£ÙÙØ§Ù'); return; }
  ...
  if (!hasAnyMapping) { alert('Ø§Ø®ØªØ± Ø¹ÙÙ Ø§ÙØ£ÙÙ Ø­ÙÙ ÙØ§Ø­Ø¯ ÙÙØ§Ø³ØªÙØ±Ø§Ø¯'); return; }
  ...
  document.getElementById('vlPreviewStats').innerHTML =
    '<span style="background:#dcfce7;color:#15803d;padding:3px 10px;border-radius:12px;font-size:13px;margin:0 4px;">â Ø³ÙØªÙ ØªØ­Ø¯ÙØ«: '+matched.length+' ÙÙØªØ¬</span>' +
    '<span style="background:#fee2e2;color:#b91c1c;padding:3px 10px;border-radius:12px;font-size:13px;margin:0 4px;">â ØºÙØ± ÙÙØ¬ÙØ¯: '+notFound.length+'</span>' +
    '<span style="background:#f3f4f6;color:#374151;padding:3px 10px;border-radius:12px;font-size:13px;margin:0 4px;">â ÙØ§ ØªØºÙÙØ±: '+noChange.length+'</span>';

  var fieldLabel = {cost:'Ø§ÙØªÙÙÙØ©', priceBefore:'Ø§ÙØ³Ø¹Ø± ÙØ¨Ù', priceAfter:'Ø§ÙØ³Ø¹Ø± Ø¨Ø¹Ø¯', category:'Ø§ÙÙØ¦Ø©', family:'Ø§ÙÙØ¬ÙÙØ¹Ø©'};
  var tbody = '';
  matched.forEach(function(r) {
    var changesHtml = Object.entries(r.changes).map(function(e){
      var lbl = fieldLabel[e[0]] || e[0];
      var old = r.prod ? (r.prod[e[0]]||'â') : 'â';
      return '<div style="font-size:11px;"><span style="color:var(--text-muted);">'+lbl+':</span> <span style="text-decoration:line-through;color:#dc2626;">'+old+'</span> â <strong style="color:#15803d;">'+e[1]+'</strong></div>';
    }).join('');
  ...
  if (notFound.length) {
    notFound.forEach(function(r){
      ...
      tbody += '<td colspan="2" style="padding:8px;font-size:12px;color:#dc2626;">â ØºÙØ± ÙÙØ¬ÙØ¯ ÙÙ Ø§ÙÙØ®Ø²ÙÙ</td>';
      tbody += '</tr>';
    });
  }
  document.getElementById('vlPreviewBody').innerHTML = tbody || '<tr><td colspan="3" style="text-align:center;padding:20px;color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ ØªØºÙÙØ±Ø§Øª</td></tr>';
...
function applyVlookup(matched) {
  if (!matched || !matched.length) { alert('ÙØ§ ØªÙØ¬Ø¯ ØªØºÙÙØ±Ø§Øª ÙÙØªØ·Ø¨ÙÙ'); return; }
  var inv = getInv();
  matched.forEach(function(r) {
  ...
  alert('ØªÙ ØªØ­Ø¯ÙØ« ' + matched.length + ' ÙÙØªØ¬ Ø¨ÙØ¬Ø§Ø­');
}
...
// ââââââââââââââââââââââââââââââââââââââââââââââ
// WAREHOUSE (Ø§ÙÙØ®Ø²Ù Ø§ÙØ±Ø¦ÙØ³Ù) MODULE
// ââââââââââââââââââââââââââââââââââââââââââââââ
var _whTrItems = [];
...
function renderWarehousePage() {
  ...
  document.getElementById('wh-value').textContent = fmt(totalValue) + ' Ø¬';
  document.getElementById('wh-low').textContent   = lowItems;

  // Table
  var tbody = '';
  if (!items.length) {
    tbody = '<tr><td colspan="8" style="text-align:center;padding:30px;color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ Ø£ØµÙØ§Ù â Ø£Ø¶Ù ÙÙØªØ¬Ø§Øª Ø£Ù Ø§Ø³ØªÙØ±Ø¯ ÙÙ Excel</td></tr>';
  } else {
    items.forEach(function(p, i) {
      ...
      tbody += '<td style="padding:8px;text-align:center;font-size:12px;">'+(p.category||'â')+'</td>';
      tbody += '<td style="padding:8px;text-align:center;font-size:12px;">'+(p.family||'â')+'</td>';
      tbody += '<td style="padding:8px;text-align:center;font-weight:700;color:'+qtyClr+';">'+(p.qty||0)+(low?' â ï¸':'')+'</td>';
      ...
      tbody += '<button class="btn btn-outline btn-sm" style="font-size:11px;padding:3px 8px;" onclick="openWhTransferModal(\''+p.code+'\')">ð¤ ØªØ­ÙÙÙ</button>';
      tbody += '</td>';
      tbody += '</tr>';
    });
  }
  ...
  var histHtml = '';
  if (!transfers.length) {
    histHtml = '<p style="color:var(--text-muted);font-size:13px;">ÙØ§ ØªÙØ¬Ø¯ ØªØ­ÙÙÙØ§Øª Ø¨Ø¹Ø¯</p>';
  } else {
    ...
    histHtml += '<thead><tr style="background:var(--bg-secondary);"><th style="padding:8px;text-align:right;">Ø§ÙØªØ§Ø±ÙØ®</th><th style="padding:8px;text-align:right;">ÙÙ</th><th style="padding:8px;text-align:right;">Ø¥ÙÙ</th><th style="padding:8px;text-align:center;">Ø§ÙØ£ØµÙØ§Ù</th><th style="padding:8px;">ÙÙØ§Ø­Ø¸Ø©</th></tr></thead><tbody>';
    transfers.forEach(function(t){
      ...
      histHtml += '<td style="padding:7px 8px;text-align:center;">'+(t.items?.length||0)+' ØµÙÙ</td>';
      histHtml += '<td style="padding:7px 8px;font-size:12px;color:var(--text-muted);">'+(t.note||'â')+'</td>';
      histHtml += '</tr>';
    });
...
function whTrSearchProducts() {
  ...
  resultsDiv.innerHTML = matches.map(function(p){
    return '<div onclick="whTrAddItem('+JSON.stringify(p).replace(/"/g,"'")+'); this.parentNode.style.display=\'none\';" style="padding:9px 12px;cursor:pointer;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;" onmouseover="this.style.background=\'#f0f9ff\'" onmouseout="this.style.background=\'white\'">'
      ...
      +'<span style="font-size:12px;color:var(--text-muted);">ÙØªØ§Ø­: <strong style="color:#1d4ed8;">'+p.qty+'</strong></span>'
      +'</div>';
  }).join('');
}
...
function renderWhTrItems() {
  ...
  if (!_whTrItems.length) { div.innerHTML='<p style="color:var(--text-muted);font-size:13px;text-align:center;padding:12px;">Ø§Ø¨Ø­Ø« Ø¹Ù Ø§ÙÙÙØªØ¬Ø§Øª ÙØ£Ø¶ÙÙØ§</p>'; return; }
  ...
  _whTrItems.forEach(function(item, idx){
    ...
    html2 += '<div style="font-size:11px;color:var(--text-muted);">ÙØªØ§Ø­ ÙÙ Ø§ÙÙØ®Ø²Ù: '+item.maxQty+'</div></div>';
    html2 += '<div style="display:flex;align-items:center;gap:6px;">';
    html2 += '<button onclick="whTrChangeQty('+idx+',-1)" class="btn btn-gray" style="padding:2px 8px;font-size:14px;min-width:28px;">â</button>';
    ...
    html2 += '<button onclick="whTrRemoveItem('+idx+')" style="background:#fee2e2;border:none;border-radius:5px;padding:3px 8px;cursor:pointer;color:#dc2626;">â</button>';
    html2 += '</div></div>';
  });
  html2 += '</div>';
  div.innerHTML = html2;
...
function confirmWhTransfer() {
  ...
  if (!to) { alert('Ø§Ø®ØªØ± Ø§ÙÙØ±Ø¹'); return; }
  if (!_whTrItems.length) { alert('Ø£Ø¶Ù Ø£ØµÙØ§Ù ÙÙØªØ­ÙÙÙ'); return; }
  ...
  for (var k=0; k<_whTrItems.length; k++) {
    ...
    if (!p || p.qty < it.qty) {
      alert('Ø§ÙÙÙÙØ© Ø§ÙÙØ·ÙÙØ¨Ø© ÙÙ "'+it.name+'" ØºÙØ± ÙØªØ§Ø­Ø© ÙÙ Ø§ÙÙØ®Ø²Ù Ø§ÙØ±Ø¦ÙØ³Ù (ÙØªØ§Ø­: '+(p?p.qty:0)+')');
      return;
    }
  }
  ...
  var record = {
    ...
    from: 'wh', to: to,
    fromName: branches['wh'] || 'ð­ Ø§ÙÙØ®Ø²Ù Ø§ÙØ±Ø¦ÙØ³Ù',
    toName:   branches[to]   || BRANCH_DEFAULTS[to],
  ...
  renderWarehousePage();
  alert('â ØªÙ Ø§ÙØªØ­ÙÙÙ Ø¨ÙØ¬Ø§Ø­\nÙÙ: Ø§ÙÙØ®Ø²Ù Ø§ÙØ±Ø¦ÙØ³Ù â Ø¥ÙÙ: '+(record.toName)+'\n'+_whTrItems.length+' ØµÙÙ');
}
...
// ââ Cashier: edit item price ââââââââââââââââââââââââââââ
function editItemPrice(code) {
  var item = cart.find(function(i){ return i.code === code; });
  if (!item) return;
  var newPrice = parseFloat(prompt('Ø§ÙØ³Ø¹Ø± Ø§ÙØ¬Ø¯ÙØ¯ ÙÙ "' + item.name + '" (Ø§ÙØ³Ø¹Ø± Ø§ÙØ­Ø§ÙÙ: ' + item.price + ')', item.price));
...
// ââ Cashier: send for approval ââââââââââââââââââââââââââ
function sendForApproval() {
  if (!cart.length) { alert('Ø§ÙÙØ§ØªÙØ±Ø© ÙØ§Ø±ØºØ©'); return; }
  var hasModified = cart.some(function(i){ return i.priceModified; });
  if (!hasModified) { openPayment(); return; }
  var note = prompt('ÙÙØ§Ø­Ø¸Ø© ÙÙÙØ¯ÙØ± (Ø§Ø®ØªÙØ§Ø±Ù):', '') || '';
  var { total } = cartTotals();
  var request = {
    id: Date.now(),
    date: new Date().toISOString(),
    cashier: currentUser || 'ÙØ§Ø´ÙØ±',
    branchId: currentBranch,
  ...
  clearCart();
  updateApprovalBadge();
  alert('â ØªÙ Ø¥Ø±Ø³Ø§Ù Ø§ÙÙØ§ØªÙØ±Ø© ÙÙÙØ¯ÙØ±\nØ³ÙØªÙ Ø¥Ø´Ø¹Ø§Ø±Ù Ø¹ÙØ¯ Ø§ÙÙÙØ§ÙÙØ©');
}

// ââ Admin: open approvals panel âââââââââââââââââââââââââ
...
function renderApprovalsList() {
  ...
  if (!list.length) {
    el.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ Ø·ÙØ¨Ø§Øª ÙØ¹ÙÙØ© â</div>';
    return;
  }
  el.innerHTML = list.map(function(req){
    var itemsHtml = req.items.filter(function(i){ return i.priceModified; }).map(function(i){
      return '<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #f3f4f6;">'
        ...
        + '<span style="text-decoration:line-through;color:#9ca3af;">'+fmt(i.originalPrice)+' Ø¬</span>'
        + ' â <span style="color:#dc2626;font-weight:700;">'+fmt(i.price)+' Ø¬</span>'
        + ' Ã '+i.qty
        + '</span></div>';
    }).join('');
    return '<div style="background:white;border:1px solid var(--border);border-radius:10px;padding:16px;margin-bottom:12px;">'
      ...
      + '<div><div style="font-weight:700;font-size:14px;">ð ÙØ§ØªÙØ±Ø© #'+req.id+'</div>'
      + '<div style="font-size:12px;color:var(--text-muted);">'+req.date.slice(0,16).replace('T',' ')+' â '+req.branchName+' â '+req.cashier+'</div></div>'
      + '<div style="font-size:20px;font-weight:800;color:#1d4ed8;">'+fmt(req.total)+' Ø¬</div>'
      + '</div>'
      + '<div style="margin-bottom:10px;">'+itemsHtml+'</div>'
      + (req.note ? '<div style="background:#fef9c3;padding:6px 10px;border-radius:6px;font-size:12px;margin-bottom:10px;">ð¬ '+req.note+'</div>' : '')
      + '<div style="display:flex;gap:8px;">'
      + '<input id="adminNote_'+req.id+'" class="form-control" placeholder="ÙÙØ§Ø­Ø¸Ø© ÙÙÙØ§Ø´ÙØ± (Ø§Ø®ØªÙØ§Ø±Ù)" style="flex:1;font-size:12px;" />'
      + '<button onclick="approveRequest('+req.id+')" class="btn btn-success">â ÙÙØ§ÙÙØ©</button>'
      + '<button onclick="rejectRequest('+req.id+')" class="btn btn-danger btn-sm">â Ø±ÙØ¶</button>'
      + '</div></div>';
  }).join('');
}
...
function approveRequest(id) {
  ...
  updateApprovalBadge();
  renderApprovalsList();
  alert('â ØªÙØª Ø§ÙÙÙØ§ÙÙØ© â Ø³ÙØªÙ Ø¥Ø´Ø¹Ø§Ø± Ø§ÙÙØ§Ø´ÙØ±');
}
...
// ââ Badge update ââââââââââââââââââââââââââââââââââââââââ
...
// ââ Cashier: watch for approvals ââââââââââââââââââââââââ
...
function checkForApprovedCarts() {
  ...
  document.getElementById('approvalToastMsg').textContent = approved.length + ' ÙØ§ØªÙØ±Ø© Ø¬Ø§ÙØ²Ø© ÙÙØ¥ØªÙØ§Ù';
  if (toast) { toast.style.display = 'block'; setTimeout(function(){ toast.style.display='none'; }, 12000); }
}

// ââ Cashier: open approved carts ââââââââââââââââââââââââ
...
function renderApprovedCartsList() {
  ...
  if (!myApproved.length) {
    el.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text-muted);">ÙØ§ ØªÙØ¬Ø¯ ÙÙØ§ØªÙØ± ÙØ¹ØªÙØ¯Ø©</div>';
    return;
  }
  el.innerHTML = myApproved.map(function(req){
    var itemsHtml = req.items.map(function(i){
      return '<div style="display:flex;justify-content:space-between;font-size:13px;padding:4px 0;">'
        ...
        + '<span style="font-weight:700;'+(i.priceModified?'color:#dc2626;':'')+'">'+fmt(i.price)+' Ø¬</span>'
        + '</div>';
    }).join('');
    return '<div style="background:white;border:2px solid #10b981;border-radius:10px;padding:14px;margin-bottom:10px;">'
      ...
      + '<div><div style="font-weight:700;">â ÙØ¹ØªÙØ¯Ø© â '+req.branchName+'</div>'
      + '<div style="font-size:12px;color:var(--text-muted);">'+req.date.slice(0,16).replace('T',' ')+'</div></div>'
      + '<div style="font-size:20px;font-weight:800;color:#1d4ed8;">'+fmt(req.total)+' Ø¬</div>'
      + '</div>'
      + '<div style="margin-bottom:10px;">'+itemsHtml+'</div>'
      + (req.adminNote ? '<div style="background:#d1fae5;padding:6px 10px;border-radius:6px;font-size:12px;margin-bottom:8px;">ð¬ '+req.adminNote+'</div>' : '')
      + '<button onclick="resumeApprovedCart('+req.id+')" class="btn btn-success" style="width:100%;">ð ØªØ­ÙÙÙ Ø§ÙÙØ§ØªÙØ±Ø© ÙØ¥ØªÙØ§Ù Ø§ÙØ¨ÙØ¹</button>'
      + '</div>';
  }).join('');
}
...
function resumeApprovedCart(id) {
  ...
  if (!req) return;
  if (cart.length && !confirm('Ø³ÙØªÙ Ø§Ø³ØªØ¨Ø¯Ø§Ù Ø§ÙÙØ§ØªÙØ±Ø© Ø§ÙØ­Ø§ÙÙØ©. ÙÙ ØªØ±ÙØ¯ Ø§ÙÙØªØ§Ø¨Ø¹Ø©Ø')) return;
...
// ââ Firebase listener for approvals ââââââââââââââââââââ
...
function renderSuspApprovals() {
  ...
  if (!list.length) {
    el.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-muted);">â ÙØ§ ØªÙØ¬Ø¯ Ø·ÙØ¨Ø§Øª ÙØ¹ÙÙØ©</div>';
    return;
  }
  el.innerHTML = list.map(function(req){
    ...
    var itemsHtml = modItems.map(function(i){
      return '<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #f3f4f6;font-size:13px;">'
        +'<span style="font-weight:600;">'+i.name+' Ã '+i.qty+'</span>'
        +'<span><span style="text-decoration:line-through;color:#9ca3af;">'+fmt(i.originalPrice)+' Ø¬</span>'
        +' â <span style="color:#dc2626;font-weight:700;">'+fmt(i.price)+' Ø¬</span></span>'
        +'</div>';
    }).join('');
    return '<div style="background:white;border:1px solid #fecaca;border-right:4px solid #ef4444;border-radius:10px;padding:16px;margin-bottom:12px;">'
      +'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">'
      +'<div>'
      +'<div style="font-weight:700;font-size:14px;">âï¸ Ø·ÙØ¨ ØªØ¹Ø¯ÙÙ Ø³Ø¹Ø±</div>'
      +'<div style="font-size:12px;color:var(--text-muted);">'+req.date.slice(0,16).replace('T',' ')+' â '+req.branchName+' â '+req.cashier+'</div>'
      +'</div>'
      +'<div style="font-size:20px;font-weight:800;color:#1d4ed8;">'+fmt(req.total)+' Ø¬</div>'
      +'</div>'
      +'<div style="margin-bottom:10px;">'+itemsHtml+'</div>'
      +(req.note?'<div style="background:#fef9c3;padding:6px 10px;border-radius:6px;font-size:12px;margin-bottom:10px;">ð¬ '+req.note+'</div>':'')
      +'<div style="display:flex;gap:8px;align-items:center;">'
      +'<input id="sAdminNote_'+req.id+'" class="form-control" placeholder="ÙÙØ§Ø­Ø¸Ø© ÙÙÙØ§Ø´ÙØ± (Ø§Ø®ØªÙØ§Ø±Ù)" style="flex:1;font-size:12px;" />'
      +'<button onclick="approveSuspRequest('+req.id+')" class="btn btn-success">â ÙÙØ§ÙÙØ©</button>'
      +'<button onclick="rejectSuspRequest('+req.id+')" class="btn btn-danger btn-sm">â Ø±ÙØ¶</button>'
      +'</div>'
      +'</div>';
  }).join('');
}
...
function updateHomeClock() {
  ...
  var months = ['ÙÙØ§ÙØ±','ÙØ¨Ø±Ø§ÙØ±','ÙØ§Ø±Ø³','Ø£Ø¨Ø±ÙÙ','ÙØ§ÙÙ','ÙÙÙÙÙ','ÙÙÙÙÙ','Ø£ØºØ³Ø·Ø³','Ø³Ø¨ØªÙØ¨Ø±','Ø£ÙØªÙØ¨Ø±','ÙÙÙÙØ¨Ø±','Ø¯ÙØ³ÙØ¨Ø±'];
  ...
  if (greet) {
    var h = now.getHours();
    var g = h < 12 ? 'ð ØµØ¨Ø§Ø­ Ø§ÙØ®ÙØ±' : h < 18 ? 'âï¸ ÙØ³Ø§Ø¡ Ø§ÙØ®ÙØ±' : 'ð ÙØ³Ø§Ø¡ Ø§ÙÙÙØ±';
    greet.textContent = g;
  }
}
...
async function confirmFirstRun() {
  ...
  if (p1.length < 4) { msg.textContent = 'ÙÙÙØ© Ø§ÙÙØ±ÙØ± ÙØµÙØ±Ø© Ø¬Ø¯Ø§Ù (4 Ø£Ø­Ø±Ù Ø¹ÙÙ Ø§ÙØ£ÙÙ)'; return; }
  if (p1 !== p2)     { msg.textContent = 'ÙÙÙØªØ§ Ø§ÙÙØ±ÙØ± ØºÙØ± ÙØªØ·Ø§Ø¨ÙØªÙÙ'; return; }
  ...
  showToast('â ØªÙ ØªØ¹ÙÙÙ ÙÙÙØ© ÙØ±ÙØ± Ø§ÙÙØ¯ÙØ± â Ø³Ø¬ÙÙ Ø§ÙØ¯Ø®ÙÙ Ø§ÙØ¢Ù');
}
...
// EXPENSE APPROVAL REQUESTS â Ø·ÙØ¨Ø§Øª Ø§Ø¹ØªÙØ§Ø¯ Ø§ÙÙØµØ§Ø±ÙÙ
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
function saveExpenseRequest() {
  ...
  showToast('â ØªÙ Ø¥Ø±Ø³Ø§Ù Ø·ÙØ¨ Ø§ÙÙØµØ±ÙÙ â Ø¨Ø§ÙØªØ¸Ø§Ø± Ø§Ø¹ØªÙØ§Ø¯ Ø§ÙØ¥Ø¯Ø§Ø±Ø©');
}
function approveExpenseRequest(id) {
  if (!confirm('ØªØ£ÙÙØ¯ Ø§Ø¹ØªÙØ§Ø¯ ÙØ°Ø§ Ø§ÙÙØµØ±ÙÙØ')) return;
  ...
  expenses.push({ id:'exp_'+Date.now(), type:'branch', branchId:req.branchId, category:req.category,
    ...
    note:(req.note?req.note+' â ':'')+'ÙØ¹ØªÙØ¯ ÙÙ '+req.by, by:currentUser, createdAt:Date.now() });
  setExpenses(expenses);
  updateExpReqBadge(); renderExpensesPage(); showToast('â ØªÙØª Ø§ÙÙÙØ§ÙÙØ© ÙØªØ³Ø¬ÙÙ Ø§ÙÙØµØ±ÙÙ');
}
function rejectExpenseRequest(id) {
  const list = getExpenseRequests(); const req = list.find(r=>r.id===id); if (!req) return;
  req.status = 'rejected'; req.reviewedBy = currentUser; req.reviewedAt = Date.now();
  setExpenseRequests(list); updateExpReqBadge(); renderExpensesPage(); showToast('â ØªÙ Ø±ÙØ¶ Ø§ÙØ·ÙØ¨');
}
...
function renderExpenseRequestsPane() {
  ...
  if (!reqs.length) { cont.innerHTML = '<p style="color:var(--text-muted);text-align:center;padding:32px">ÙØ§ ØªÙØ¬Ø¯ Ø·ÙØ¨Ø§Øª ÙØµØ§Ø±ÙÙ</p>'; return; }
  cont.innerHTML = reqs.map(r => '<div style="background:var(--bg-secondary);border:1px solid var(--border);border-radius:10px;padding:14px 18px;margin-bottom:10px;display:flex;align-items:center;gap:12px;flex-wrap:wrap;">' +
    '<div style="flex:1;min-width:200px">' +
      '<div style="font-weight:700;font-size:15px">'+(EXP_CATS[r.category]||r.category)+' â <span style="color:#3d8fff">'+(r.amount||0).toLocaleString()+' Ø¬</span></div>' +
      '<div style="font-size:12px;color:var(--text-muted);margin-top:4px">ð¬ '+getBranchName(r.branchId)+' Â· ð '+r.date+(r.note?' Â· '+r.note:'')+'</div>' +
      '<div style="font-size:11px;color:var(--text-muted);margin-top:3px">ð¤ '+r.by+' Â· '+new Date(r.createdAt).toLocaleString('ar-EG')+'</div>' +
    '</div>' +
    '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">' +
      '<span style="padding:5px 12px;border-radius:20px;font-size:12px;font-weight:700;background:'+(r.status==='pending'?'rgba(255,193,7,.15)':r.status==='approved'?'rgba(40,167,69,.15)':'rgba(220,53,69,.15)')+';color:'+(r.status==='pending'?'#e09000':r.status==='approved'?'#28a745':'#dc3545')+'">' +
        (r.status==='pending'?'â³ Ø¨Ø§ÙØªØ¸Ø§Ø± Ø§ÙØ§Ø¹ØªÙØ§Ø¯':r.status==='approved'?'â ÙØ¹ØªÙØ¯':'â ÙØ±ÙÙØ¶') +
      '</span>' +
      (isAdmin && r.status==='pending' ?
        '<button class="btn" style="background:#28a745;color:#fff;font-size:12px;padding:6px 14px" onclick="approveExpenseRequest(\''+r.id+'\')">â ÙØ¨ÙÙ</button>' +
        '<button class="btn" style="background:#dc3545;color:#fff;font-size:12px;padding:6px 14px" onclick="rejectExpenseRequest(\''+r.id+'\')">â Ø±ÙØ¶</button>' : '') +
...
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// LEAVE & PERMISSION REQUESTS â Ø·ÙØ¨Ø§Øª Ø§ÙØ¥Ø¬Ø§Ø²Ø§Øª ÙØ§ÙØ£Ø°ÙÙØ§Øª
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
function saveLeaveRequest() {
  ...
  const type    = document.getElementById('leaveReqType').value;
  if (!empName) { alert('Ø§Ø®ØªØ± Ø§ÙÙÙØ¸Ù'); return; }
  if (!date)    { alert('Ø£Ø¯Ø®Ù Ø§ÙØªØ§Ø±ÙØ®'); return; }
  const rec = {
    id: 'leaveq_' + Date.now(),
    branchId: currentBranch,
  ...
  showToast('â ØªÙ Ø¥Ø±Ø³Ø§Ù Ø§ÙØ·ÙØ¨ â Ø¨Ø§ÙØªØ¸Ø§Ø± ÙÙØ§ÙÙØ© Ø§ÙØ¥Ø¯Ø§Ø±Ø©');
}
function approveLeaveRequest(id) {
  if (!confirm('ØªØ£ÙÙØ¯ Ø§ÙÙÙØ§ÙÙØ© Ø¹ÙÙ ÙØ°Ø§ Ø§ÙØ·ÙØ¨Ø')) return;
  const list = getLeaveRequests(); const req = list.find(r=>r.id===id); if (!req) return;
  req.status = 'approved'; req.reviewedBy = currentUser; req.reviewedAt = Date.now();
  setLeaveRequests(list);
  if (req.type === 'leave') {
    saveAttendanceRecord(req.empName, req.date, 'absent', '', '', 'Ø¥Ø¬Ø§Ø²Ø© ÙØ¹ØªÙØ¯Ø©');
  } else {
    const notes = (req.fromTime && req.toTime) ? 'Ø¥Ø°Ù Ø§ÙØµØ±Ø§Ù '+req.fromTime+' - '+req.toTime : 'Ø¥Ø°Ù Ø§ÙØµØ±Ø§Ù ÙØ¹ØªÙØ¯';
    saveAttendanceRecord(req.empName, req.date, 'late', req.fromTime||'', req.toTime||'', notes);
  }
  updateLeaveReqBadge(); renderHRPage(); showToast('â ØªÙØª Ø§ÙÙÙØ§ÙÙØ© ÙØªØ³Ø¬ÙÙ Ø§ÙØ­Ø¶ÙØ±');
}
function rejectLeaveRequest(id) {
  const list = getLeaveRequests(); const req = list.find(r=>r.id===id); if (!req) return;
  req.status = 'rejected'; req.reviewedBy = currentUser; req.reviewedAt = Date.now();
  setLeaveRequests(list); updateLeaveReqBadge(); renderHRPage(); showToast('â ØªÙ Ø±ÙØ¶ Ø§ÙØ·ÙØ¨');
}
...
function renderLeaveRequestsPane() {
  ...
  const reqs = getLeaveRequests()
    .filter(r => isAdmin ? true : r.branchId === currentBranch)
    .sort((a,b) => b.createdAt - a.createdAt);
  if (!reqs.length) { cont.querySelector('#leaveReqList').innerHTML = '<p style="color:var(--text-muted);text-align:center;padding:32px">ÙØ§ ØªÙØ¬Ø¯ Ø·ÙØ¨Ø§Øª Ø¥Ø¬Ø§Ø²Ø§Øª Ø£Ù Ø£Ø°ÙÙØ§Øª</p>'; return; }
  const TYPE_LABELS = { leave: 'ðï¸ Ø¥Ø¬Ø§Ø²Ø© ÙÙÙ ÙØ§ÙÙ', permission: 'ð Ø¥Ø°Ù Ø§ÙØµØ±Ø§Ù' };
  cont.querySelector('#leaveReqList').innerHTML = reqs.map(r =>
    ...
    '<div style="flex:1;min-width:200px">' +
      '<div style="font-weight:700;font-size:15px">ð¤ '+r.empName+' â <span style="color:#3d8fff">'+(TYPE_LABELS[r.type]||r.type)+'</span></div>' +
      '<div style="font-size:12px;color:var(--text-muted);margin-top:4px">ð¬ '+getBranchName(r.branchId)+' Â· ð '+r.date+(r.type==='permission'&&r.fromTime?' Â· ð '+r.fromTime+'â'+r.toTime:'')+(r.reason?' Â· '+r.reason:'')+'</div>' +
      '<div style="font-size:11px;color:var(--text-muted);margin-top:3px">ð¤ '+r.by+' Â· '+new Date(r.createdAt).toLocaleString('ar-EG')+'</div>' +
    '</div>' +
    '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">' +
      '<span style="padding:5px 12px;border-radius:20px;font-size:12px;font-weight:700;background:'+(r.status==='pending'?'rgba(255,193,7,.15)':r.status==='approved'?'rgba(40,167,69,.15)':'rgba(220,53,69,.15)')+';color:'+(r.status==='pending'?'#e09000':r.status==='approved'?'#28a745':'#dc3545')+'">' +
        (r.status==='pending'?'â³ Ø¨Ø§ÙØªØ¸Ø§Ø± Ø§ÙÙÙØ§ÙÙØ©':r.status==='approved'?'â ÙÙØ§ÙÙ Ø¹ÙÙÙ':'â ÙØ±ÙÙØ¶') +
      '</span>' +
      (isAdmin && r.status==='pending' ?
        '<button class="btn" style="background:#28a745;color:#fff;font-size:12px;padding:6px 14px" onclick="approveLeaveRequest(\''+r.id+'\')">â ÙØ¨ÙÙ</button>' +
        '<button class="btn" style="background:#dc3545;color:#fff;font-size:12px;padding:6px 14px" onclick="rejectLeaveRequest(\''+r.id+'\')">â Ø±ÙØ¶</button>' : '') +
...
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// PRINTING â Ø·Ø§Ø¨Ø¹Ø§Øª (ESC/POS USB + Barcode + A4)
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
function updatePrinterStatusUI() {
  ...
  if (el) el.innerHTML = _serialPort
    ? '<span style="color:#28a745;font-weight:700">ð¢ Ø§ÙØ·Ø§Ø¨Ø¹Ø© ÙØªØµÙØ©</span>'
    : '<span style="color:#dc3545">ð´ ØºÙØ± ÙØªØµÙØ©</span>';
}
async function connectReceiptPrinter() {
  if (!('serial' in navigator)) {
    alert('ÙØªØµÙØ­Ù ÙØ§ ÙØ¯Ø¹Ù Web Serial API\nØ§Ø³ØªØ®Ø¯Ù Chrome Ø£Ù Edge Ø¹ÙÙ Ø³Ø·Ø­ Ø§ÙÙÙØªØ¨ ÙÙØ·');
    return;
  }
  try {
    ...
    updatePrinterStatusUI();
    showToast('â ØªÙ Ø§ÙØ§ØªØµØ§Ù Ø¨Ø§ÙØ·Ø§Ø¨Ø¹Ø©');
  } catch(e) {
    _serialPort = null; _serialWriter = null;
    showToast('â ÙØ´Ù Ø§ÙØ§ØªØµØ§Ù: ' + e.message);
  }
}
async function disconnectPrinter() {
  try {
    ...
    updatePrinterStatusUI();
    showToast('â ØªÙ ÙØ·Ø¹ Ø§ÙØ§ØªØµØ§Ù');
  } catch(e) { showToast('â Ø®Ø·Ø£: ' + e.message); }
}
async function sendToSerial(data) {
  if (!_serialWriter) return false;
  try { await _serialWriter.write(data); return true; }
  catch(e) { showToast('â Ø®Ø·Ø£ ÙÙ Ø§ÙØ·Ø¨Ø§Ø¹Ø©: ' + e.message); return false; }
}
...
async function printReceiptESCPOS(sale) {
  ...
  if (ok) showToast('â ØªÙ Ø§ÙØ¥Ø±Ø³Ø§Ù ÙÙØ·Ø§Ø¨Ø¹Ø©');
}
async function testReceiptPrint() {
  if (!_serialWriter) { alert('Ø§ÙØ·Ø§Ø¨Ø¹Ø© ØºÙØ± ÙØªØµÙØ© â Ø§Ø¶ØºØ· Ø§ØªØµØ§Ù Ø£ÙÙØ§Ù'); return; }
  await printReceiptESCPOS({
    id:'TEST001', date: new Date().toISOString().slice(0,10),
    salesperson:'Ø§Ø®ØªØ¨Ø§Ø±', items:[{name:'ÙÙØªØ¬ ØªØ¬Ø±ÙØ¨Ù',qty:2,price:50}],
    total:100, paid:100
  });
}

// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// FINGERPRINT IMPORT â Ø§Ø³ØªÙØ±Ø§Ø¯ Ø¨ÙØ§ÙØ§Øª Ø¬ÙØ§Ø² Ø§ÙØ¨ØµÙØ©
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
function handleFingerprintFile(input) {
  ...
  logEl.innerHTML = '<div style="color:var(--text-muted)">â³ Ø¬Ø§Ø±Ù ÙØ±Ø§Ø¡Ø© Ø§ÙÙÙÙ...</div>';
  const reader = new FileReader();
  reader.onload = function(e) {
    ...
    rows.forEach((cols, idx) => {
      ...
      saveAttendanceRecord(empName, date, status, checkIn, checkOut, 'Ø§Ø³ØªÙØ±Ø§Ø¯ Ø§ÙØ¨ØµÙØ©');
      imported++; logs.push('â '+empName+' â '+date+(checkIn?' ('+checkIn+'-'+checkOut+')':''));
    });
    logEl.innerHTML =
      '<div style="color:#28a745;font-weight:700;margin-bottom:8px">ØªÙ Ø§ÙØ§Ø³ØªÙØ±Ø§Ø¯: '+imported+' Ø³Ø¬Ù</div>'+
      (skipped?'<div style="color:#ffc107;margin-bottom:8px">ØªØ®Ø·Ù: '+skipped+' Ø³Ø·Ø±</div>':'')+
      '<div style="max-height:180px;overflow-y:auto;font-size:12px;color:var(--text-muted)">'+
        logs.slice(0,50).join('<br>')+(logs.length>50?'<br>... Ù '+(logs.length-50)+' Ø¢Ø®Ø±':'')+
      '</div>';
    if (imported > 0) {
      showToast('â ØªÙ Ø§Ø³ØªÙØ±Ø§Ø¯ '+imported+' Ø³Ø¬Ù Ø­Ø¶ÙØ±');
...
function downloadFingerprintTemplate() {
  const csv = 'Ø§ÙØ§Ø³Ù,Ø§ÙØªØ§Ø±ÙØ®,ÙÙØª Ø§ÙØ­Ø¶ÙØ±,ÙÙØª Ø§ÙØ§ÙØµØ±Ø§Ù\nØ£Ø­ÙØ¯ ÙØ­ÙØ¯,2026-07-04,09:00,17:00\nÙØ­ÙØ¯ Ø¹ÙÙ,2026-07-05,09:15,17:30\n';
  const blob = new Blob(['\uFEFF'+csv], {type:'text/csv;charset=utf-8'});
  const a = document.createElement('a'); a.href=URL.createObjectURL(blob);
  a.download='ÙÙÙØ°Ø¬_Ø¨ÙØ§ÙØ§Øª_Ø§ÙØ¨ØµÙØ©.csv'; a.click();
}

// âââ Badge updates on showPage ââââââââââââââââââââââââââââââââââââââââ



/* âââââââââââââââââââââââââââââââââââââââââââââââ
   HOME PAGE â ROLE-BASED ICON GRID
   âââââââââââââââââââââââââââââââââââââââââââââââ */
