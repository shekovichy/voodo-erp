...
function renderCustomers() {
  ...
  const vipCount   = custs.filter(c => (c.totalSpent||0) >= thresh).length;
  ...
  showConfirmModal('حذف هذا العميل؟', function() {
    ...
    if (c) {
      const changes = buildAuditDiff(
        { name: c.name, phone: c.phone || '-', totalSpent: fmt(c.totalSpent || 0) + ' ج', points: c.loyaltyPoints || 0 },
        {},
...
function openCustomerProfile(id) {
  const c = getCustomers().find(x => x.id === id);
  if (!c) return;
  // getSales() has no guaranteed order (insertion/listener-merge order, not
  // chronological) — "آخر الفواتير" below used to just slice(0,20) off the
  // front of that unsorted list, so it could show arbitrary old invoices
  // instead of the customer's actual most recent ones. Sorted once here,
  // newest first, and used for both that list and "top products".
